...
function ani_start()
	...
	ani_window = guiCreateWindow(width-560, height-(height/2), 550, 312, "Özel Animasyonlar", false)
	...
	ani_tab_all = guiCreateTab("All", ani_tab_panel)
		...
		ani_close = guiCreateButton(325, 221, 93, 21, "Close", false, ani_tab_all)
			addEventHandler("onClientGUIClick", ani_close, animpanel_hide, false)
	...
	ani_tab_favourites = guiCreateTab("Favourites", ani_tab_panel)
		...
		ani_close_fav = guiCreateButton(325, 221, 93, 21, "Close", false, ani_tab_favourites)
			addEventHandler("onClientGUIClick", ani_close_fav, animpanel_hide, false)
...
function applyAnimation(block,ani,loop)
	for customAnimationBlockIndex, customAnimationBlock in ipairs (IFP) do 
		if (block == customAnimationBlock.blockName) then
			setPedAnimation (localPlayer, tostring(block), tostring(ani), -1, loop, true, true)
			serverToAnim(block, ani)
			...
			triggerServerEvent("AnimationSet", localPlayer, tostring(block), tostring(ani), loop)
		end
	end
end
...
addEventHandler("onClientResourceStart", resourceRoot, ani_start)
...
addEventHandler ("onClientCustomAnimationSet", root,
	function (source, blockName, animationName, loop)
...
function serverToAnim(block,number)
...
addEvent("anim -- receive", true)
addEventHandler("anim -- receive", root,
    function(e, block,number, thisPlayer)

        if e == localPlayer then
        	setPedAnimation(thisPlayer,block,number,-1, false)
        end
    end
)
