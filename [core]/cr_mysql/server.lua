...
local username = "root"
local password = "SvZuFaBI5qWMa2OC"
local database = "game"
local port = 3306
...
function connectToDatabase(res)
	...
	conn = dbConnect("mysql", "dbname=" .. database .. ";host=" .. hostname, username, password, "autoreconnect=1")
	if (not MySQLConnection) then
		...
		outputDebugString("Yeni MYSQL agina baglanildi.")
	end
...
function logSQLError(str)
	...
	exports['cr_logs']:logMessage("MYSQL ERROR [QUERY] " .. message .. " [ERROR] " .. mysql_errno(MySQLConnection) .. ": " .. mysql_error(MySQLConnection), 24)
end
...
function query(str)
	...
	if sqllog then
		exports['cr_logs']:logMessage(str, 24)
	end
...
local resources = {
	["mysql"] = getResourceFromName("cr_mysql"),
	["shop-system"] = getResourceFromName("cr_shop-system"),
	["vehicle-manager"] = getResourceFromName("cr_vehicle-manager"),
}
