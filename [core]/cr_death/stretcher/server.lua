...
addEventHandler("stretcher:hasPlayerStretcherSpawned", root, hasPlayerStretcherSpawned)
...
addEventHandler("stretcher:isPedStretcherOccupied", root, isPedStretcherOccupied)
		...
		if vehicle and isElement(vehicle) then
			...
			if (isElement(patient)) then --and getElementAttachedTo(stretcherArray[playerElement]) == patient
				...
				if not warpResult then
					outputChatBox("The " .. tostring(exports.cr_global:getVehicleName(vehicle)) .. " does not have space for any more patients.", playerElement, 255, 0, 0)
				else
...
addEventHandler("stretcher:destroyStretcher", root, destroyStretcher)
...
function leaveStretcher(playerElement)
	...
	if  stretcherArray[playerElement] then
		triggerClientEvent(playerElement, "stretcher:getPositionInFrontOfElement", root, playerElement, false, "leave")
	end
end
addEvent("stretcher:leaveStretcher", true)
addEventHandler("stretcher:leaveStretcher", root, leaveStretcher)
...
function takeStretcher(stretcher, playerElement)
	...
	if  isElement(stretcher) and getElementType(stretcher) == "object" then
		if not stretcherArray[playerElement] then
			...
			triggerClientEvent(playerElement, "stretcher:getPositionInFrontOfElement", root, playerElement, false, "take") 
		end
	end
end
addEvent("stretcher:takeStretcher", true)
addEventHandler("stretcher:takeStretcher", root, takeStretcher)
...
function createStretcher(playerElement, vehicle)
	...
	if hasPlayerStretcherSpawned(playerElement) then
		if vehicle then
			exports.cr_global:sendLocalMeAction(playerElement, "puts the stretcher inside the " .. tostring(exports.cr_global:getVehicleName(vehicle)) .. ".")
		else
			exports.cr_global:sendLocalMeAction(playerElement, "puts the stretcher inside.")
		end
		destroyStretcher(playerElement, vehicle)
	else
		if vehicle then
			exports.cr_global:sendLocalMeAction(playerElement, "takes out a stretcher from the " .. tostring(exports.cr_global:getVehicleName(vehicle)) .. ".")
		else
			exports.cr_global:sendLocalMeAction(playerElement, "takes out a stretcher.")
		end
		triggerClientEvent(playerElement, "stretcher:getPositionInFrontOfElement", root, playerElement, vehicle) 
	end
end
addEvent("stretcher:createStretcher", true)
addEventHandler("stretcher:createStretcher", root, createStretcher)
...
addEventHandler("stretcher:getPositionInFrontOfElement", root, getPositionInFrontOfElement)
		...
		if (isElement(stretcherArray[player])) then
			...
			if isPedStretcherOccupied(player) then
				...
				if (isElement(patient)) then --and getElementAttachedTo(stretcherArray[player]) == patient
					...
					triggerClientEvent(patient, "setPlayerInsideInterior", root, interiortable, root)
...
addEventHandler ("onVehicleStartEnter", root, checkPedEnterVehicleWithStretcher)
...
function movePedOntoStretcher(	targetElement, playerElement)
	...
	exports.cr_global:applyAnimation(targetElement, "CRACK", "crckdeth2", -1, true)
...
addEventHandler("stretcher:movePedOntoStretcher", root, movePedOntoStretcher)
...
function takePedFromStretcher(	targetElement, playerElement)
	...
	exports.cr_global:removeAnimation(targetElement)
...
addEventHandler("stretcher:takePedFromStretcher", root, takePedFromStretcher)
