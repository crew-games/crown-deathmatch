...
function acceptDeath(thePlayer, victimDropItem)
	if getElementData(thePlayer, "dead") == 1 then
		...
		setElementData(thePlayer, "protection", true)
        if isTimer(timers[thePlayer]) then
            killTimer(timers[thePlayer]) 
            timers[thePlayer] = nil
        end
        timers[thePlayer] = setTimer(disableSpawnProtection, seconds * 1000, 1, thePlayer)
	end
end
...
function disableSpawnProtection(thePlayer)
    setElementData(thePlayer, "protection", false)
    if isTimer(timers[thePlayer]) then
        killTimer(timers[thePlayer]) 
        timers[thePlayer] = nil
    end
...
function logMe(message)
	local logMeBuffer = getElementData(root, "killog") or {}
	local r = getRealTime()
	...
	setElementData(root, "killog", logMeBuffer)
end
...
local tedaviYeri = createColSphere(1591.8984375, 1798.3076171875, 2083.376953125, 6)
setElementInterior(tedaviYeri, 0)
setElementDimension(tedaviYeri, 53)

function tedaviOl(thePlayer)
	if isElementWithinColShape(thePlayer, tedaviYeri) then
		if getElementData(thePlayer, "injured") == 1 then
			if exports.cr_global:takeMoney(thePlayer, 100) then
				setElementData(thePlayer, "injured", 0)
				setElementHealth(thePlayer, 100)
...
addCommandHandler("tedaviol", tedaviOl)
		...
		if targetPlayer then
			...
			if targetPlayer then
				if getElementData(targetPlayer, "loggedin") == 1 then
					if getElementData(targetPlayer, "injured") == 1 then
						setElementData(targetPlayer, "injured", 0)
...
addCommandHandler("tedaviet", tedaviEt)
	...
	if exports.cr_integration:isPlayerDeveloper(thePlayer) then
		for _, player in ipairs(getElementsByType("player")) do
			if getElementData(player, "dead") == 1 then
				...
				setElementData(player, "injured", 0) 
