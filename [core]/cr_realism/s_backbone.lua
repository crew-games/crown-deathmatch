...
function updateBackbone()
	...
	triggerClientEvent(root, "realism:backbone.removeBackboneItem", source, source)
		...
		if (weapon and weapon ~= currentWeapon) then
			...
			if ammo > 0 then
				if isBackboneAllowed(weapon) then
					...
					triggerClientEvent(root, "realism:backbone.addBackboneItem", source, source, weaponmodels[weapon])
...
addEventHandler("onPlayerWeaponSwitch", root, updateBackbone)
addEventHandler("onCharacterLogin", root, updateBackbone)
addEventHandler("onPedWeaponSwitch", root, updateBackbone)
...
addEventHandler("realism:backbone.request", root, requestBackBone)
