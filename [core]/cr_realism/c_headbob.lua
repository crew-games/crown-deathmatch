function bobHead()
	if not isPedOnFire(localPlayer) then
		...
		setPedLookAt(source, x, y, z, 3000)
	end
end
addEventHandler("onClientRender", root, bobHead)
...
function showColBox()
	if (show) then
		removeEventHandler("onClientPreRender", root, drawBox)
		show = false
	else
		addEventHandler("onClientPreRender", root, drawBox)
		show = true
	end
end
...
function togGta1()
	if (cam) then
		removeEventHandler("onClientPreRender", root, gta1cam)
		setCameraTarget(player)
		cam = false
	else
		setPedRotation(player, 360)
		setPedCameraRotation(player, 360)
		addEventHandler("onClientPreRender", root, gta1cam)
		cam = true
	end
end
