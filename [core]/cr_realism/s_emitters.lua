...
function createEmitter(thePlayer, commandName, type)
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (type) or (type - 0 > 3) or (type - 0 < 1) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Emitter Type]", thePlayer, 255, 194, 14)
			...
			if tonumber(type) == 1 or tonumber(type) == 2 then
				...
				end, 10000*3, 1) -- give admin 30 seconds to avoid being damaged by fire / Farid
			end
...
function delEmitter(thePlayer, commandName, id)
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer)) then
		if not (id) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Emitter ID]", thePlayer, 255, 194, 14)
		else
