...
addEventHandler("ck:info", root,
	function()
		if CKs[source] then
			triggerClientEvent(client, "ck:show", client, CKs[source].text, exports.cr_integration:isPlayerTrialAdmin(client) and CKs[source].name)
		end
	end
)
...
addEventHandler("onResourceStart", resourceRoot, loadAllCorpses)

function getNearbyCKs(thePlayer, commandName)
local theTeam = getPlayerTeam(thePlayer)
local factionType = getElementData(theTeam, "type")
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer)) or factionType == 4 then
