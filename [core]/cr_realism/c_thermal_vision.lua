...
function doVision()
	...
	if not (tV) then
		removeEventHandler("onClientRender", root, doVision)
		return
	end
		...
		if isElementStreamedIn(value) and (isElementOnScreen(value)) and (tV~=value) then
			...
			if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
				...
				if (tx) then
					...
					local realName = exports.cr_global:getVehicleName(value)
		...
		if (getElementModel(source)==497 or getElementModel(source)==487) then
			...
			if (playerFaction == 1 or playerFaction == 2 or playerFaction == 45 or playerFaction == 59 or vehID == 8772) then	
				if (seat == 0 or seat == 1) then
					addEventHandler("onClientRender", root, doVision)
				end
			end
		end
	end
end
addEventHandler("onClientVehicleEnter", root, applyVision)
...
addEventHandler("onClientVehicleExit", root, removeVision)

addEventHandler("onClientResourceStart", resourceRoot,
    function (startedRes)
		local setVehicle = getPedOccupiedVehicle(localPlayer)
		if setVehicle then
        applyVision(localPlayer, 0, setVehicle)
		end
    end
);