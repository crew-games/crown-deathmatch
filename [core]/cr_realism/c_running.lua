local run = 0
local down = 0

addEventHandler("onClientPreRender", root, 
	function(slice)
		if (not isPedInVehicle(localPlayer) and getControlState("sprint")) or down > 0 then
			run = run + slice
			if run >= 40000 then
				if isPedOnGround(localPlayer) then
					exports.cr_global:applyAnimation(localPlayer, "FAT", "idle_tired", 5000, true, false, true)
					run = 19000
				else
					toggleControl('sprint', false)
					setTimer(toggleControl, 5000, 1, 'sprint', true)
				end
			end
			if not getControlState("sprint") then
				down = math.max(0, down - slice)
			else
				down = 500
			end
		else
			if run > 0 then
				run = math.max(0, run - math.ceil(slice / 3))
			end
		end
	end
)