	...
	if task ~= oldTask then
		if theVehicle then
			if task == "TASK_SIMPLE_CAR_OPEN_LOCKED_DOOR_FROM_OUTSIDE" then
				triggerServerEvent("onVehicleRemoteAlarm", theVehicle, localPlayer)
				theVehicle = nil
...
addEventHandler("onClientRender", root, checkAlarm)
...
addEventHandler("startCarAlarm", root, carAlarm)
...
addEventHandler("onClientVehicleStartEnter", root, updateCar)
...
function doCarAlarm(vehicle)
	if isElement(vehicle) then
		...
		if exports['cr_global']:hasItem(vehicle, 130) == true then
