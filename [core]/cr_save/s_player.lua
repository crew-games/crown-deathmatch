...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			local update = dbExec(mysql:getConnection(), "UPDATE characters SET x='" .. mysql:escape_string(x) .. "', y='" .. mysql:escape_string(y) .. "', z='" .. mysql:escape_string(z) .. "', rotation='" .. mysql:escape_string(rot) .. "', health='" .. mysql:escape_string(health) .. "', armor='" .. mysql:escape_string(armor) .. "', dimension_id='" .. mysql:escape_string(dimension) .. "', interior_id='" .. mysql:escape_string(interior) .. "', " .. mysql:escape_string(money) .. mysql:escape_string(businessprofit) .. "lastlogin=NOW(), lastarea='" .. mysql:escape_string(zone) .. "', timeinserver='" .. mysql:escape_string(timeinserver) .. "', alcohollevel='" ..  mysql:escape_string(tostring(alcohollevel))  .. "', kills='" .. mysql:escape_string(kills) .. "', deaths='" .. mysql:escape_string(deaths) .. "' WHERE id=" .. mysql:escape_string(getElementData(player, "dbid")))
