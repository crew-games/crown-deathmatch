...
function radio(source, radioID, message)
	...
	if hasRadio or getElementType(source) == "ped" or radioID == -2 then
		...
		elseif theChannel > 1 or radioID < 0 then
			--triggerClientEvent (source, "playRadioSound", root)
...
addEventHandler("onPlayerChat", root, chatMain)
        ...
        if (...) then
            ...
            for _, player in ipairs(getElementsByType("player")) do
            	if getElementData(player, "loggedin") == 1 then
					triggerClientEvent(player, "infobox.addBox", player, "announcement", adminName .. ": " .. message, 10000, "top-center")
				end
            end
        else 
...
function pmPlayer(thePlayer, commandName, who, ...)
	...
	if who and message and getElementData(thePlayer, "loggedin") == 1 then
		...
		if (targetPlayer) then
			...
			if getElementData(thePlayer, "adminjailed") and not exports.cr_integration:isPlayerAdmin1(targetPlayer) then
			...
			if not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and not exports.cr_integration:isPlayerSeniorAdmin(targetPlayer) then
...
function localOOC(thePlayer, commandName, ...)
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		...
		elseif (muted==1) then
			...
			if exports.cr_integration:isPlayerLeaderAdmin(thePlayer) and getElementData(thePlayer, "duty_admin") == 1 and getElementData(thePlayer, "hiddenadmin") == 0 and not getElementData(thePlayer, "supervising") then
				r,b , g = 0, 185, 255
			...
			elseif exports.cr_integration:isPlayerSupporter(thePlayer) and getElementData(thePlayer, "supervising") then
				r, b, g = 100, 149, 237
				setElementData(thePlayer, "supervisorBchat", true)
			elseif exports.cr_integration:isPlayerSupporter(thePlayer) and not getElementData(thePlayer, "supervising") then
				r, b, g = 196, 255, 255
				setElementData(thePlayer, "supervisorBchat", false)
			end
			local playerName = getPlayerName(thePlayer):gsub("_", " ")
	...
	if (factionType == 2) then
		...
		elseif #message == 0 then
			...
			local username = getPlayerName(thePlayer)
				...
				triggerClientEvent (value, "playHQSound", root)
		...
		if exports.cr_integration:isPlayerLeaderAdmin(thePlayer) then
			if (...) then
				...
				for i, player in ipairs(getElementsByType("player")) do
					if exports.cr_integration:isPlayerLeaderAdmin(player) then
...
addCommandHandler("u", managementChat, false, false)
...
function supporterChat(thePlayer, commandName, ...)
	if getElementData(thePlayer, "loggedin") == 1 then
		if exports.cr_integration:isPlayerSupporter(thePlayer) or exports.cr_integration:isPlayerTrialAdmin(thePlayer) then
			if (...) then
				...
				local accountName = getElementData(thePlayer, "account:username")
				for i, player in ipairs(getElementsByType("player")) do
					if exports.cr_integration:isPlayerSupporter(player) or exports.cr_integration:isPlayerTrialAdmin(player) then
...
addCommandHandler("g", supporterChat, false, false)
	...
	if message then
		for _, player in ipairs(getElementsByType("player")) do
			if exports.cr_integration:isPlayerSupporter(player) or exports.cr_integration:isPlayerTrialAdmin(player) then
				...
				if not hidesa then
					outputChatBox(message, player, 116, 192, 252)
				end
			end
		end
		return "SUCCESS"
			...
			if (...) then
				...
				for _, player in ipairs(getElementsByType("player")) do
					if exports.cr_integration:isPlayerSupporter(player) or exports.cr_integration:isPlayerTrialAdmin(player) then
						local hidesa = getElementData(player, "hidesa") or false
						if not hidesa then
							outputChatBox("[CDP] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. ": " .. message, player, 255, 212, 59)
						end
					end
				end
				
				callRemote("185.160.30.55:22005", getResourceName(getThisResource()), "sendInterserverChatMessage", function(responseData, errorID)
				...
				end, "[CDP] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. ": " .. message)
			else 
				outputChatBox("KULLANIM: /" .. commandName .. " [İleti]", thePlayer, 255, 194, 14)
			end
		else
...
function toggleSupporterChat(thePlayer, commandName)
	if exports.cr_integration:isPlayerSupporter(thePlayer) or exports.cr_integration:isPlayerTrialAdmin(thePlayer) then
...
addCommandHandler("togg", toggleSupporterChat, false, false)
addCommandHandler("toggleg", toggleSupporterChat, false, false)
addCommandHandler("togh", toggleSupporterChat, false, false)
addCommandHandler("toggleh", toggleSupporterChat, false, false)
		...
		if not (targetPlayerNick) or not (amount) or not tonumber(amount) then
			...
			if targetPlayer then
				...
				if (distance<=10) then
					...
					elseif (hoursplayed<5) and (amount>50) and not exports.cr_integration:isPlayerTrialAdmin(thePlayer) and not exports.cr_integration:isPlayerTrialAdmin(targetPlayer) and not exports.cr_integration:isPlayerSupporter(thePlayer) and not exports.cr_integration:isPlayerSupporter(targetPlayer) then
					...
					elseif exports.cr_global:hasMoney(thePlayer, amount) then
						if hoursplayed < 5 and not exports.cr_integration:isPlayerTrialAdmin(targetPlayer) and not exports.cr_integration:isPlayerTrialAdmin(thePlayer) and not exports.cr_integration:isPlayerSupporter(targetPlayer) and not exports.cr_integration:isPlayerSupporter(thePlayer) then
...
addEventHandler("onPlayerQuit", root, removeBigEars)
...
addEventHandler("onPlayerPrivateMessage", root, disableMsg)
...
function isPlayerStaff(thePlayer)
	--if exports.cr_integration:isPlayerSupporter(thePlayer) then return true end
...
function chatTemizle(thePlayer, commandName)
    if exports.cr_integration:isPlayerAdmin3(thePlayer) then
        for i = 0, 100 do
			outputChatBox(" ", root)
		end
		outputChatBox("[!]#FFFFFF " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili sohbeti temizledi.", root, 0, 255, 0, true)
		outputChatBox("[!]#FFFFFF Keyifli DM'ler ve eğlenceler dileriz.", root, 0, 255, 0, true)
		exports.cr_discord:sendMessage("temizle-log", "[TEMIZLE] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili chati temizledi.")
    end
end
addCommandHandler("temizle", chatTemizle)
