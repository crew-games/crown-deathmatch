...
addEventHandler("market.buyVIP", root, function(vip, day, price)
	...
	if getElementData(source, "balance") >= price then
		if day >= 7 then
			...
			exports.cr_infobox:addBox(source, "error", "Minimum 7 günlük VIP üyeliği satın alabilirsiniz.")
		end
	else
		exports.cr_infobox:addBox(source, "error", "Yeterli bakiyeniz yok.")
	end
...
addEventHandler("market.buyLuckBox", root, function(id, volume, price)
	if client ~= source then return end
	if getElementData(source, "balance") >= price then
		if price > 0 then
			if volume < 100 then
...
addEvent("market.buyPrivateBadge", true)
addEventHandler("market.buyPrivateBadge", root, function(value, price)
	if client ~= source then return end
	if getElementData(source, "balance") >= price then
		if price > 0 then
			if exports.cr_items:hasSpaceForItem(source, 82, value) then
				exports.cr_global:giveItem(source, 82, value)
				takeBalance(source, price)
				exports.cr_infobox:addBox(source, "success", "Başarıyla " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Özel Rozet satın aldınız.")
				exports.cr_global:sendMessageToAdmins("[MARKET] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Özel Rozet satın aldı.")
				exports.cr_discord:sendMessage("market-log", "[MARKET] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Özel Rozet satın aldı.")
			else
				exports.cr_infobox:addBox(source, "error", "Envanteriniz dolu.")
			end
		end
...
addEvent("market.buyElitePass", true)
addEventHandler("market.buyElitePass", root, function(price)
	if client ~= source then return end
	if getElementData(source, "balance") >= price then
		if price > 0 then
			if getElementData(source, "pass_type") ~= 2 then
				setElementData(source, "pass_type", 2)
				dbExec(mysql:getConnection(), "UPDATE characters SET pass_type = 2 WHERE id = ?", getElementData(source, "dbid"))
				takeBalance(source, price)
				exports.cr_infobox:addBox(source, "success", "Başarıyla " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Elite Pass satın aldınız.")
				exports.cr_global:sendMessageToAdmins("[MARKET] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Elite Pass satın aldı.")
				exports.cr_discord:sendMessage("market-log", "[MARKET] " .. getPlayerName(source):gsub("_", " ") .. " isimli oyuncu " .. exports.cr_global:formatMoney(price) .. " TL karşılığında Elite Pass satın aldı.")
			else
				exports.cr_infobox:addBox(source, "error", "Zaten Elite Pass'iniz var.")
			end
		end
	else
		exports.cr_infobox:addBox(source, "error", "Yeterli bakiyeniz yok.")
