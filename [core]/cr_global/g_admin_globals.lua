...
function getPlayerFullAdminTitle(thePlayer)
	if isElement(thePlayer) then
		...
		if exports.cr_integration:isPlayerTrialAdmin(thePlayer) then
			...
			title = text
		elseif exports.cr_integration:isPlayerSupporter(thePlayer) then
			title = "Rehber"
...
function isStaff(thePlayer)
	if thePlayer and isElement(thePlayer) and getElementType(thePlayer) == "player" then
		return exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer)
	else
		return false
	end
end
...
function isSupporterOnDuty(thePlayer)
	if thePlayer and isElement(thePlayer) and getElementType(thePlayer) == "player" then
		return exports.cr_integration:isPlayerSupporter(thePlayer) and (getElementData(thePlayer, "duty_supporter") == 1)
	else
		return false
	end
end
