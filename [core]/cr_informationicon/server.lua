...
function makeInformationIcon(thePlayer, commandName, ...)
	...
	if ... then
		...
		if query then
			...
			outputChatBox("KULLANIM: /addii [Information]", thePlayer, 255, 194, 14)
		end
	end
end
...
addEventHandler("onResourceStop", resourceRoot, saveAllInformationIcons)
...
function deleteInformationIcon(thePlayer, commandName, ID)
	if (exports.cr_integration:isPlayerSeniorAdmin(thePlayer)) then
		if tonumber(ID) then
			...
			outputChatBox("KULLANIM: /delii [ID]", thePlayer, 255, 194, 14)
		end
	end
end
