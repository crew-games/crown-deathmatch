...
function showsize(thePlayer)
	if not exports.cr_integration:isPlayerDeveloper(thePlayer) then
		return false
	end
...
--[[--Disabled because it will be detected as infinite running script and will be aborted by MTA anyway. Below is an alternative/better solution / Farid
addEventHandler("onResourceStart", root, 
	function(resource)
		allocateElement(resource == getThisResource() and root or source)
	end
)
]]
...
addEventHandler("onResourceStart", resourceRoot, function() --Farid
	local loaded = exports.cr_data:load("poolTable")
	if loaded then
		poolTable = loaded
	end
	loaded = exports.cr_data:load("indexedPools")
	if loaded then
		indexedPools = loaded
	end
