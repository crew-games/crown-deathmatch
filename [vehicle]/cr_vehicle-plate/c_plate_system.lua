...
function getPaperFromTony(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then --if it's a right-click on a object
        ...
        if (pedName == "Tony Johnston") then
            ...
            showCursor(true)
            local row = exports.cr_rightclick:addRow("Buy DMV transaction paper - 100$")
            addEventHandler("onClientGUIClick", row,  function (button, state)
            	if exports.cr_global:hasMoney(localPlayer, 100) and getElementData(localPlayer, "loggedin") == 1 then
                	triggerServerEvent("givePaperToSellVehicle", resourceRoot, localPlayer)
					showCursor(false)
				else
...
addEventHandler("onClientClick", root, getPaperFromTony, true)
...
function cBeginGUI()
	...
	greetingWindow = guiCreateStaticImage(x, y, width, height, ":cr_ui/public/images/window_body.png", false)
...
addEventHandler("cBeginPlate", root, cBeginGUI)

function fetchPlateList()
	if (source==plates) then
		destroyElement(greetingWindow)
		triggerServerEvent("platePedTalk", localPlayer, 3)
		triggerServerEvent("vehicle-plate-system:list", localPlayer)
		showCursor(false)
	end
end
...
function fetchRegisterList()
	if (source==register) then
		destroyElement(greetingWindow)
		triggerServerEvent("platePedTalk", localPlayer, 3)
		triggerServerEvent("vehicle-plate-system:registerlist", localPlayer)
		showCursor(false)
	end
end
...
function PlateWindow(vehicleList)
	...
	for i,r in ipairs(vehicleList) do
		...
		guiGridListSetItemText(vehlist, row, ov, exports.cr_global:getVehicleName(v) , false, false) --tostring(getVehicleNameFromModel(vm))
...
addEventHandler("vehicle-plate-system:clist", root, PlateWindow)
...
addEventHandler("vehicle-plate-system:rlist", root, registerWindow)
...
function editPlateWindow()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		efinalWindow = guiCreateStaticImage(x, y, width, height, ":cr_ui/public/images/window_body.png", false)
		...
		addEventHandler("onClientGUIClick", submitNP, function ()
			if source == submitNP then
				...
				triggerServerEvent("sNewPlates", localPlayer, data, vehid)
...
function updateRegistration()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		if exports.cr_global:hasMoney(localPlayer,  state == "Unregistered" and 175 or 50) then
...
function closeWindow()
	...
	elseif (source==neither) then
		...
		toggleAllControls(true)
		triggerEvent("onClientPlayerWeaponCheck", localPlayer)
		triggerServerEvent("platePedTalk", localPlayer, 4)
