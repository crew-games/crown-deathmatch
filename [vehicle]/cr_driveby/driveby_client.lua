...
addEventHandler("onClientResourceStart",resourceRoot,
	function()
		bindKey ("mouse2", "down", "driveby-system >> toggle", "")
		bindKey ("e", "down", "driveby-system >> new", "1")
		bindKey ("q", "down", "driveby-system >> previous", "-1")
...
function toggleDriveby()
	...
	local windowState = getElementData(getPedOccupiedVehicle(localPlayer), "vehicle:windowstat") or 0
	if not isPedInVehicle(localPlayer) then return end
	if windowState == 0 then
		outputChatBox(">>#FFFFFF Camdan çıkabilmek için aracınızın camının açık olması gerekiyor.", 255, 0, 0, true)
		return
	end
	local protection = getElementData(localPlayer, "protection") or false
	if protection then
		outputChatBox(">>#FFFFFF Korumada iken camdan çıkamazsınız.", 255, 0, 0, true)
		return
	end
	...
	if equipedWeapon == 0 then
		...
		local prevw,nextw = next(getBoundKeys ("driveby-system >> previous")),next(getBoundKeys ("driveby-system >> new"))
...
addCommandHandler ("driveby-system >> toggle", toggleDriveby)
...
addCommandHandler ("driveby-system >> new", switchDrivebyWeapon)
addCommandHandler ("driveby-system >> previous", switchDrivebyWeapon)
...
local function onWeaponSwitchWhileDriveby (prevSlot, curSlot)
	if isPedDoingGangDriveby(source) then
		...
		for key, weaponID in ipairs(weaponsTable) do
			local slot = getSlotFromWeapon (weaponID)
			local weapon = getPedWeapon (source, curSlot)
