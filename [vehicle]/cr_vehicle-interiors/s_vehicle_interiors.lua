	...
	function()
		for key, value in ipairs(getElementsByType("vehicle")) do
			if getElementData(value, "entrance") then
				setElementData(value, "entrance")
			end
		end
		--[[for key, value in ipairs(vehicleInteriorGates) do
			exports.cr_gates:removeGate(value)
		end--]]
	end
)
...
function add(vehicle)
	...
	if getElementModel(vehicle) == 519 then -- Shamal
		...
		--add cockpit door		
		local gate = exports["cr_gate"]:createGate(2924,1,34.5,1199.8000488281,0,0,180,-0.30000001192093,34.5,1199.8000488281,0,0,180,1,dim,0,30,9,tostring(dbid),2)
		table.insert(vehicleInteriorGates, gate)

		addInterior(vehicle,  3.755859375, 23.0126953125, 1199.6, 1)
...
function enterCockpitByKey(thePlayer, key, keyState)
	...
	if vehicle then
		...
		if (model == 577 or model == 519) then --AT-400 & Shamal
			if (getElementData(thePlayer, "duty_admin") == 1 or exports.cr_global:hasItem(thePlayer, 3, tonumber(dbid)) or (getElementData(thePlayer, "faction") > 0 and getElementData(thePlayer, "faction") == getElementData(vehicle, "faction"))) then
				allowed = true
			end
		end
		if allowed then
		...
		if getElementData(value, "dbid") == dim then
			if isVehicleLocked(value) then
				...
				if (getElementData(value, "airport.gate.connected")) then
					...
					local teleportArr = exports["cr_sfia"]:getDataForExitingConnectedPlane(gateID, value)
					if teleportArr then
		...
		if (tpd > 20000) then
			...
			for key, value in ipairs(getElementsByType("vehicle")) do
				if getElementData(value, "dbid") == vid then
					exports.cr_global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " begins to knock on the vehicle.", 255, 51, 102)
					exports.cr_global:sendLocalText(value, " * Knocks can be heard coming from inside the vehicle. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		else
			if vehicles[veh] then
				...
				if (exit) then
					exports.cr_global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " begins to knock on the vehicle.", 255, 51, 102)
					exports.cr_global:sendLocalText(exit, " * Knocks can be heard coming from the outside. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		end
	end
...
addEventHandler("onVehicleKnocking", root, vehicleKnock)
...
function seeThroughWindows(thePlayer, nx, ny, nz) -- This is for /windows
	...
	if (getElementData(thePlayer, "isInWindow") == false or not getElementData(thePlayer, "isInWindow")) and dim > 20000 then
		...
		local id = dim - 20000
		local vehicle = exports.cr_pool:getElement("vehicle", tonumber(id))
		local x, y, z = getElementPosition(vehicle)
...
addEventHandler("seeThroughWindows", root, seeThroughWindows)
...
function updateView(thePlayer) -- This checks if the vehicle changed dim/int. If it did, it returns the player back into normal position so his int/dim is not fucked up. He will need to type /windows again.
	if getElementData(thePlayer, "isInWindow") == true then
		...
		if int ~= getElementData(thePlayer, "isInWindow:initialint") and dim ~= getElementData(thePlayer, "isInWindow:initialdim") then
			triggerEvent("seeThroughWindows", root, thePlayer)
		end
	end
end
addEvent("updateWindowsView", true)
addEventHandler("updateWindowsView", root, updateView)
