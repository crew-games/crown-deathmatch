...
function getVehicleOwner(vehicle)
	...
	if faction > 0 then
		return getTeamName(exports.cr_pool:getElement('team', faction))
	else
		return call(getResourceFromName("cr_cache"), "getCharacterName", getElementData(vehicle, "owner")) or "N/A"
	end
end

--[[
function createForumThread(fTitle, fContent)
	local posterUsername = "AI"
	local posterID = 186
	local createInForumID = 544
	
	local content = "[CENTER][IMG]http://crowndeathmatch.net/wp-content/uploads/2014/01/OwlLogo4.png[/IMG]\n[SIZE=5][FONT=impact]" .. fTitle .. "[/FONT][/SIZE][/CENTER]" .. fContent
	content = mysql:escape_string(content)
	
	local firstID = exports.cr_mysql:forum_query_insert_free("INSERT INTO post SET  parentid = '0', username = '" .. posterUsername .. "', userid = '" .. posterID .. "', title = '" .. fTitle .. "', dateline = unix_timestamp(), pagetext = '" .. content .. "', allowsmilie = '0', showsignature = '0', ipaddress = '127.0.0.1', iconid = '0', visible = '1', attach = '0', infraction = '0', reportthreadid = '0'")
	
	local seccondID = exports.cr_mysql:forum_query_insert_free("INSERT INTO thread SET `force_read_usergroups`='', `force_read_forums`='', title = '" .. fTitle .. "', firstpostid = '" .. firstID .. "', lastpost = unix_timestamp(), forumid = '" .. createInForumID .. "', pollid = '0', open = '1', replycount = '0', postercount = '1', hiddencount = '0', deletedcount = '0', postusername = '" .. posterUsername .. "', postuserid = '" .. posterID .. "', lastposter = '" .. posterUsername .. "', lastposterid = '" .. posterID .. "', dateline = unix_timestamp(), views = '0', iconid = '0', visible = '1', sticky = '0', votenum = '0', votetotal = '0', attach = '0' ")
	
	exports.cr_mysql:forum_query_free("UPDATE post SET threadid = '" .. seccondID .. "' WHERE postid = '" .. firstID .. "'")
	exports.cr_mysql:forum_query_free("update `user` set posts = posts + 1 where userid = '" .. posterID .. "' ")
	exports.cr_mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = '" .. posterUsername .. "', lastposterid='" .. posterID .. "', lastpostid='" .. firstID .. "', lastthread='" .. fTitle .. "', lastthreadid='" .. seccondID .. "', threadcount = threadcount + 1 WHERE forumid = '" .. createInForumID .. "'")
	return "http://forums.crowndeathmatch.net/showthread.php/" .. seccondID
end
]]