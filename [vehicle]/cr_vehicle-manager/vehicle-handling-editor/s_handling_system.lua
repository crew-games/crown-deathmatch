...
local startDriveTestConvo = {
	"Tamam, hadi yapalım!", 
	"Tamam gidelim!", 
	"Hadi yuvarlanalım. Ne için bekliyorsun?", 
	"Utanmayın, sadece deneyin.", 
	"Eminim hoşuna gider, hadi gidelim!"
}

local finishDriveTestConvo = {
	"Yani ne düşünüyorsun?", 
	"Fena değil, değil mi?", 
	"Kesinlikle bunu sipariş etmelisiniz!", 
	"Bu yüzden..?", 
	"Harcamaya değer, değil mi?"
}

local orderVehicleConvo = {
	"Peki, bu model stoğumuza girer girmez size sms göndereceğiz.", 
}
...
function createDealerNPCs(which)
	...
	if which == getThisResource() or which == "Nicholas Davis" then
		--outputDebugString("Created")
		dealershipPed[11] = createPed(101, 161.728515625, -1930.533203125, 3.7734375)
		setPedRotation(dealershipPed[11], 83.58502197266)
	...
	if which == getThisResource() or which == "Anthony Anderson" then
		--outputDebugString("Created")
		dealershipPed[12] = createPed(97, 161.7412109375, -1927.439453125, 3.7734375)
		setPedRotation(dealershipPed[12], 83.971862792969)
...
addEventHandler("onResourceStart",resourceRoot,createDealerNPCs)
...
function deleteUniqueHandling(theVehicle, player)
	...
	if dbid and tonumber(dbid) then
		...
		if existed then
			...
			exports.cr_logs:dbLog(player, 6, { player, theVehicle }, " Reset unique handling for vehicle #" .. dbid .. ".")
			exports.cr_global:sendMessageToAdmins("[VEHICLE-MANAGER]: " .. getElementData(player, "account:username") .. " has reset unique handling for vehicle #" .. dbid .. ".")
			exports["cr_vehicle"]:reloadVehicle(tonumber(dbid))
...
function saveHandling(theVehicle1, saveToSQL, who)
	...
	if saveToSQL == 0 then -- SAVE TO ALL INSTANCES
		...
		exports.cr_logs:dbLog(who, 6, { who }, "CHANGED HANDLINGS OF ALL INSTANCES OF VEHICLE SHOP ID #" .. id)
		exports.cr_global:sendMessageToAdmins("[VEHICLE-MANAGER]: " .. getElementData(who, "account:username") .. " has changed handlings of all instances of vehicle shop id #" .. id)
		exports.cr_discord:sendMessage("editveh-log", "[HAND] " .. exports.cr_global:getPlayerFullAdminTitle(who) .. " isimli yetkili [" .. dbid .. "] ID'li aracın handını değiştirdi.")
	elseif saveToSQL == 1 then --SAVE TO 1 INSTANCE (UNIQUE)
		...
		local veh = exports.cr_pool:getElement('vehicle', tonumber(dbid))
		...
		if not row or not row.id then -- NOT EXISTED
			...
			mysql:query_free("UPDATE `vehicles_custom` SET `handling`='" .. toSQL(sHandling) .. "', `updatedby`='" .. getElementData(who, "account:id") .. "', `updatedate`=NOW() WHERE `id`='" .. tostring(dbid) .. "'")
		end
		
		outputChatBox("You have saved unique handlings for vehicle id #" .. dbid, who)
		exports.cr_logs:dbLog(who, 6, { who, theVehicle1 }, " SAVED UNIQUE HANDLINGS FOR VEHICLE ID #" .. dbid)
		exports.cr_global:sendMessageToAdmins("[VEHICLE-MANAGER]: " .. getElementData(who, "account:username") .. " saved unique handlings for vehicle id #" .. dbid)
		exports.cr_discord:sendMessage("editveh-log", "[HAND] " .. exports.cr_global:getPlayerFullAdminTitle(who) .. " isimli yetkili [" .. dbid .. "] ID'li aracın handını değiştirdi.")
	end
end
...
function reloadAllInstancesOf(vehicle_shop_id)
	...
	exports.cr_logs:dbLog(client, 6, { client }, "RELOADED " .. count .. " INSTANCES OF VEHICLE SHOP ID #" .. vehicle_shop_id .. " IN GAME")
	exports.cr_global:sendMessageToAdmins("[VEHICLE-MANAGER]: " .. getElementData(client, "account:username") .. " has reloaded " .. count .. " instances of vehicle shop id #" .. vehicle_shop_id .. " in game.")
end
...
function getInfoFromVehShopID(vehShopID)
	...
	local result = mysql:query_fetch_assoc("SELECT * FROM `vehicles_shop` WHERE `id` ='" .. exports.cr_global:toSQL(vehShopID) .. "' LIMIT 1")
	if result then
...
function createTestVehicle(vehShopID, thePed, fromVehLib)
	...
	if not fromVehLib then
		if exports.cr_global:takeMoney(client, 25) then
			exports.cr_global:sendLocalText(client, "* " .. exports.cr_global:getPlayerName(client) .. " " .. getElementData(thePed, "name") .. "'e bir miktar para verir.", 223, 174, 255, 30, {}, true)
		else
			triggerEvent("shop:storeKeeperSay", client, client, "25$ alabilir miyim lütfen?" , getElementData(thePed, "name"))
			return false
		end
	end
	
	exports.cr_global:fadeToBlack(client)
	...
	if not (veh) then
		outputChatBox("[!]#FFFFFF Geçersiz Araç ID.", client, 255, 0, 0)
		return false
	end
	...
	setElementData(client, "testdrivepos", savepos , false)
...
function loadHandlingToVeh(veh, handlingTable)
	...
	if veh and handlingTable and h then
		...
		--setVehicleHandling(veh, handlings[29][1], h[29]) -- I don't know why this isn't working in 1.4. Temporarily disable this stat / Farid
...
function finishTestDrive(veh, thePed, orderFinish, forSpawn)
	...
	for i = 0, getVehicleMaxPassengers(veh) do
		...
		if player then
			...
			if savedpos and (not forSpawn or player ~= client) then
				...
				setTimer(function()
					exports.cr_global:fadeFromBlack(player)
				end, 2000, 1)
				found = true
				break
			end
	...
	if not found and not forSpawn then
		...
		if savedpos then
			...
			setTimer(function()
				exports.cr_global:fadeFromBlack(client)
			end, 2000, 1)
		end
	end
...
function cancelOrderVehicle(shopname)
	if shopname then
		setElementData(client, "carshop:grotti:orderedvehicle:" .. shopname, false, true)
	else
	--[[
		for i, v in pairs(exports["cr_carshop"]:getCarShops()) do
			setElementData(client, "carshop:grotti:orderedvehicle:" .. (v.name), false)
		end
		]]
	end
end
