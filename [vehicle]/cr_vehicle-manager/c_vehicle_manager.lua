...
function createVehManagerWindow(data)
    if not exports.cr_integration:isPlayerTrialAdmin(localPlayer) and not exports.cr_integration:isPlayerVCTMember(localPlayer) and not exports.cr_integration:isPlayerScripter(localPlayer) then
    ...
    -- vehList1 = vehList
    gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800, 600, "Crown Deathmatch - Araç Yöneticisi", false)
    ...
    local lTotal = guiCreateLabel(460, 25, 164, 17, "Standart Araçlar: " .. exports.cr_global:formatMoney(#standardVehs), false, gWin)
    local lActive = guiCreateLabel(460, 42, 164, 17, "Benzersiz Araçlar: " .. exports.cr_global:formatMoney(#uniqueVehs), false, gWin)
    ...
    function(button)
        if button == "left" then
            ...
            local lText = guiCreateLabel(10, 22, 341, 16, "Bir araçla ilgili tüm bilgileri girin (ID, İsim, Sahibi, Model, ...):", false, wInteriorSearch)
            ...
            addEventHandler("onClientGUIClick", bGo, function(button)
                if button == "left" and wInteriorSearch then
                    triggerServerEvent("vehicleManager:Search", localPlayer, localPlayer, guiGetText(eSearch))
                    destroyElement(wInteriorSearch)
                    wInteriorSearch = nil
                    -- showCursor(false)
                    -- guiSetInputEnabled(false)
        ...
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    ...
                    if triggerServerEvent("vehicleManager:delVeh", localPlayer, localPlayer, gridID) then
                        -- closeVehManager()
                        -- triggerServerEvent("vehicleManager:openit", localPlayer, localPlayer)
                    end
                else
    ...
    function(button)
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    local gridID = guiGridListGetItemText(activeList, row, 1)
                    if triggerServerEvent("vehicleManager:gotoVeh", localPlayer, localPlayer, gridID) then
                        -- closeVehManager()
                        -- triggerServerEvent("vehicleManager:openit", localPlayer, localPlayer)
                    end
                else
    ...
    function(button)
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    local gridID = guiGridListGetItemText(activeList, row, 1)
                    if triggerServerEvent("vehicleManager:restoreVeh", localPlayer, localPlayer, gridID) then
                        -- closeVehManager()
                        -- triggerServerEvent("vehicleManager:openit", localPlayer, localPlayer)
                    end
                else
    ...
    function(button)
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    local gridID = guiGridListGetItemText(activeList, row, 1)
                    if triggerServerEvent("vehicleManager:removeVeh", localPlayer, localPlayer, gridID) then
                        -- closeVehManager()
                        -- triggerServerEvent("vehicleManager:openit", localPlayer, localPlayer)
                    end
                else
    ...
    function(button)
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    local gridID = guiGridListGetItemText(activeList, row, 1)
                    if triggerServerEvent("vehicleManager:forceSellInt", localPlayer, localPlayer, gridID) then
                        -- closeVehManager()
                        -- triggerServerEvent("vehicleManager:openit", localPlayer, localPlayer)
                    end
                else
    ...
    function(button)
        if button == "left" then
            ...
            if activeList then
                ...
                if row ~= -1 and col ~= -1 then
                    local gridID = guiGridListGetItemText(activeList, row, 1)
                    if triggerServerEvent("vehicleManager:openAdminNote", localPlayer, localPlayer, gridID) then
    ...
    function fetchSearchResults(interiorsResultList)
        if interiorsResultList then
            ...
            for _, record in ipairs(interiorsResultList) do
                ...
                guiGridListSetItemText(targetList[i], row, colVIN[i], record.show_vin == "1" and record.id or "Kaldırıldı", false, true)
                guiGridListSetItemText(targetList[i], row, colPlate[i], record.show_plate == "1" and record.plate or "Kaldırıldı", false, false)
                guiGridListSetItemText(targetList[i], row, colRegistered[i], record.registered == "1" and "Evet" or "Hayır", false, false)
                --guiGridListSetItemText(targetList[i], row, colOwner[i], exports.cr_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Unknown", false, false)
                ...
                guiGridListSetItemText(targetList[i], row, colTinted[i], record.tinted == "1" and "Evet" or "Hayır", false, false)
            end
        end
    end
...
function loadTab(vehs, i)
    for index, veh in ipairs(vehs) do
        ...
        guiGridListSetItemText(targetList[i], row, colName[i], exports.cr_global:getVehicleName(veh), false, false)
        guiGridListSetItemText(targetList[i], row, colVIN[i], getElementData(veh, "show_vin") == 1 and getElementData(veh, "dbid") or "Kaldırıldı", false, true)
        guiGridListSetItemText(targetList[i], row, colPlate[i], getElementData(veh, "show_plate") == 1 and getElementData(veh, "plate") or "Kaldırıldı", false, false)
        guiGridListSetItemText(targetList[i], row, colRegistered[i], getElementData(veh, "registered") == 1 and "Evet" or "Hayır", false, false)
        --guiGridListSetItemText(targetList[i], row, colOwner[i], exports.cr_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Unknown", false, false)
        ...
        guiGridListSetItemText(targetList[i], row, colTinted[i], getElementData(veh, "tinted") and "Evet" or "Hayır", false, false)
    end
end
...
function formatLastUsed(sec)
    if sec or not tonumber(sec) then
        ...
        return exports.cr_datetime:formatTimeInterval(sec) .. " önce"
    end
end
...
function createCheckVehWindow(result, adminTitle, history, notes1)
    ...
    guiSetInputEnabled(true)
    notes = notes1
    checkVehWindow = guiCreateWindow(sx / 2 - 300, sy / 2 - 233, 600, 466, "Vehicle Manager - Vehicle Admin Note - " .. adminTitle .. " " .. getElementData(localPlayer, "account:username"), false)
    ...
    if result[1][21] then
        ...
        function(button)
            if button == "left" then
                if triggerServerEvent("vehicleManager:removeVeh", localPlayer, localPlayer, result[1][1]) then
                    closeCheckVehWindow()
                end
            end
        end ,
        false)
        guiSetEnabled(bRemoveVeh1, exports.cr_integration:isPlayerSeniorAdmin(localPlayer))
        ...
        function(button)
            if button == "left" then
                triggerServerEvent("vehicleManager:gotoVeh", localPlayer, localPlayer, result[1][1])
            end
        end ,
        false)
        ...
        function(button)
            if button == "left" then
                if triggerServerEvent("vehicleManager:delVeh", localPlayer, localPlayer, result[1][1]) then
                    triggerServerEvent("vehicleManager:checkveh", localPlayer, localPlayer, "checkveh", result[1][1])
                end
            end
        end ,
    ...
    function(button)
        if button == "left" then
            ...
            local adminUsername = getElementData(localPlayer, "account:username")
