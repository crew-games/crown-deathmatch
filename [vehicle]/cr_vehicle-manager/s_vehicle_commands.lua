...
function togVehReg(admin, command, target, status)
	if (exports.cr_integration:isPlayerAdmin1(admin)) then
...
function unflipCar(thePlayer, commandName, targetPlayer)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer, "faction") == 4 or exports.cr_integration:isPlayerSupporter(thePlayer) then
...
function flipCar(thePlayer, commandName, targetPlayer)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer,"faction") == 4 or exports.cr_integration:isPlayerSupporter(thePlayer) then -- SFTR, working on motorbikes etc
		if not targetPlayer or not exports.cr_integration:isPlayerTrialAdmin(thePlayer) then
			if not (isPedInVehicle(thePlayer)) then
...
function gotoCar(thePlayer, commandName, id)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer) or exports.cr_integration:isPlayerScripter(thePlayer) or exports.cr_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) then
...
addCommandHandler("aracgetir", aracGetir)
...
function sendCar(thePlayer, commandName, id, toPlayer)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer) or exports.cr_integration:isPlayerVehicleConsultant(thePlayer) then
		if not (id) or not (toPlayer) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Araç ID] [player ID]", thePlayer, 255, 194, 14)
		else
			local theVehicle = exports.cr_pool:getElement("vehicle", tonumber(id))
							...
							if getElementData(theVehicle, "owner") == -2 and getElementData(theVehicle,"Impounded") == 0 then
								...
								if getElementData(theVehicle, "respawnposition") then
									...
									if (round(checkx, 6) == x) and (round(checky, 6) == y) then
										...
										setElementInterior(theVehicle, getElementData(theVehicle, "interior"))
										setElementDimension(theVehicle, getElementData(theVehicle, "dimension"))
										if (not getElementData(theVehicle, "carshop")) and (not getElementData(theVehicle, "robberyVehicle")) then
...
function respawnVehiclesStop(thePlayer, commandName)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) and isTimer(respawnTimer) then
		...
		if commandName then
			...
			outputChatBox("* " .. name .. " araç yenilenmesini durdurdu. *", root, 255, 194, 14)
		end
	end
end
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (veh) then
						...
						exports.cr_discord:sendMessage("fixveh-log","[FIXVEH] " .. exports.cr_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili (" .. targetPlayerName .. ") isimli kişinin aracını tamir etti.")
						outputChatBox("[!]#FFFFFF " .. username:gsub("_", " ") .. " isimli yetkili aracınızı tamir etti.", targetPlayer, 0, 0, 255, true)
					else
						outputChatBox("#ff2400[!]#FFFFFF Oyuncu bir araçta değil!", thePlayer, 255, 0, 0,true)
					end
				end
			end
		end
...
function fueltest(thePlayer, commandName, miktar)
	if getPedOccupiedVehicle(thePlayer) then 
		miktar = tonumber(miktar)
		if not miktar then return end 

		setElementData(getPedOccupiedVehicle(thePlayer),"fuel", miktar)
		outputChatBox("" .. miktar .. " olarak değiştirildi.", thePlayer, 255, 0, 0, true)
	end 
end 
addCommandHandler("fueltest", fueltest)
...
function getAVehicleColor(thePlayer, commandName, carid)
	if (exports.cr_integration:isPlayerSeniorAdmin(thePlayer) or exports.cr_integration:isPlayerScripter(thePlayer)) then
		if not (carid) then
			...
			if theVehicle then
				...
				if (dbid<0) then -- TEMP vehicle
					...
					if hiddenAdmin == 0 then
						...
						exports.cr_discord:sendMessage("delveh-log","[Araç ID] (#" .. dbid .. ")")
					else
...
function setVehTint(admin, command, target, status)
	if exports.cr_integration:isPlayerSeniorAdmin(admin) then
		if not (target) or not (status) then
			...
			if (targetPlayer) then
				...
				if (pv) then
					...
					if (stat == 1) then
						...
						--triggerClientEvent(root, "tintWindows", pv)
						...
						for k, arrayPlayer in ipairs(exports.cr_global:getAdmins()) do
							...
							if (logged) then
								if exports.cr_integration:isPlayerAdmin1(arrayPlayer) then
        ...
        for _, vehicle in ipairs(exports.cr_global:getNearbyElements(thePlayer, "vehicle")) do
            if vehicle ~= theVehicle then
                if getElementDimension(vehicle) ~= 33333 then
					if (not getElementData(vehicle, "carshop")) and (not getElementData(vehicle, "robberyVehicle")) then
						local seatCount = 0
