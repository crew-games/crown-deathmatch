...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		elseif (faction==-1) and (owner>0) then
			...
			if ownerName then
				if brand then
					outputChatBox("[Araba] Araç Sahibi: " .. ownerName .. " / Model: " .. model, thePlayer, 255, 194, 14)
					outputChatBox("[Araba] Sistem Fiyatı: " .. exports.cr_global:formatMoney(getElementData(source, "carshop:cost") or 0) .. "$ / Plaka: " .. getVehiclePlateText(source), thePlayer, 255, 194, 14)
					outputChatBox("[Araba] Araç Cezası: " .. exports.cr_global:formatMoney(getElementData(source, "punishment") or 0) .. "$", thePlayer, 255, 194, 14)
				end
...
function removeFromFactionVehicle(thePlayer)
	...
	if (vfaction~=-1) then
		...
		if (faction~=vfaction) and (seat==0) then
			if brand then
				outputChatBox("[Araba] Araç Sahibi: " .. factionName .. " / Model: " .. model, thePlayer, 255, 194, 14)
				outputChatBox("[Araba] Sistem Fiyatı: " .. exports.cr_global:formatMoney(getElementData(source, "carshop:cost") or 0) .. "$ / Plaka: " .. getVehiclePlateText(source), thePlayer, 255, 194, 14)
				outputChatBox("[Araba] Araç Cezası: " .. exports.cr_global:formatMoney(getElementData(source, "punishment") or 0) .. "$", thePlayer, 255, 194, 14)
			end
...
function toggleSellExceptions (thePlayer, commandName, player)
	if (exports.cr_integration:isPlayerSeniorAdmin(thePlayer)) and player then
	...
	elseif not player and (exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer)) then
...
function lockUnlockOutside(vehicle)
	if (not source or exports.cr_integration:isPlayerTrialAdmin(source)) or (getElementData(vehicle, "Impounded") or 0) == 0 and not getElementData(vehicle, "robberyVehicle") then
		local dbid = getElementData(vehicle, "dbid")
...
function setVehiclePosition(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		...
		if isElementWithinColShape(thePlayer, HospitalCol) and getElementData(thePlayer, "faction") ~= 2 and not exports.cr_integration:isPlayerTrialAdmin(thePlayer) and not exports.cr_integration:isPlayerSupporter(thePlayer) then
			outputChatBox("[!]#FFFFFF Hükumet binalarına yakın yerlere park edemezsiniz.", thePlayer, 255, 0, 0, true)
		elseif isElementWithinColShape(thePlayer, PershingSquareCol) and getElementData(thePlayer, "faction") ~= 1  and not exports.cr_integration:isPlayerTrialAdmin(thePlayer) and not exports.cr_integration:isPlayerSupporter(thePlayer) then
			outputChatBox("[!]#FFFFFF Hükumet binalarına yakın yerlere park edemezsiniz.", thePlayer, 255, 0, 0, true)
		else
...
function setVehiclePosition3(veh)
	if isElementWithinColShape(source, HospitalCol) and getElementData(source, "faction") ~= 2 and not exports.cr_integration:isPlayerTrialAdmin(source) then
	...
	elseif isElementWithinColShape(source, PershingSquareCol) and getElementData(source, "faction") ~= 1  and not exports.cr_integration:isPlayerTrialAdmin(source) then
		...
		local dbid = getElementData(veh, "dbid")
		local x, y, z = getElementPosition(veh)
		if (owner==playerid) or (exports.cr_global:hasItem(source, 3, dbid)) or (exports.cr_integration:isPlayerTrialAdmin(source) and exports.cr_logs:logMessage("[AVEHPOS] " .. getPlayerName(source) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
			if (dbid<0) then
