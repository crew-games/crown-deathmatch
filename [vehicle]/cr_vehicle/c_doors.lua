...
function openVehicleDoorGUI(vehicleElement)
	...
	if not vehicleElement then
		controllingVehicle = getPedOccupiedVehicle (localPlayer)
	else
		controllingVehicle = vehicleElement
		if controllingVehicle ~= getPedOccupiedVehicle(localPlayer) then
			local vehicle1x, vehicle1y, vehicle1z = getElementPosition (controllingVehicle)
			local player1x, player1y, player1z = getElementPosition (localPlayer)
	...
	local options = 0
	local guiPos = 30
	vControlGUI["main"] = guiCreateWindow(700,236,272,288,"Kapı Kontrolleri",false)	
	...
	vControlGUI["close"] = guiCreateButton(23,guiPos,230,guiPos,"Arayüzü Kapat",false, vControlGUI["main"])
...
addEventHandler("onClientPlayerVehicleExit", localPlayer, closeVehicleGUI)
...
function updateServerSide(theScrollBar, state)
	if vControlGUI["main"] then -- and state == "up" then
		...
		player1x, player1y, player1z = getElementPosition (localPlayer)
		if not (getPedOccupiedVehicle (localPlayer) == controllingVehicle) and not (getDistanceBetweenPoints3D (vehicle1x, vehicle1y, vehicle1z, player1x, player1y, player1z) < 5) then
			closeVehicleGUI()
			return
		end
