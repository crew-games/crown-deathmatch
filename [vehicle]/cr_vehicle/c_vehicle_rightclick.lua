...
function requestInventory(button)
	if button=="left" and not getElementData(localPlayer, "exclusiveGUI") then
		...
		elseif type(getElementData(vehicle, "Impounded")) == "number" and isVehicleImpounded(vehicle) and not exports.cr_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then
...
function showVehicleMenu()
	...
	--Name
	local vName = exports.cr_global:getVehicleName(vehicle)
	...
	if exports['cr_vehicle']:hasVehiclePlates(vehicle) and getElementData(vehicle, "show_plate") ~= 0 then -- Addded no plates support
	...
	if (getPedSimplestTask(localPlayer) == "TASK_SIMPLE_CAR_DRIVE" and getPedOccupiedVehicle(localPlayer) == vehicle) or exports.cr_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (getElementData(localPlayer, "faction") > 0 and getElementData(localPlayer, "faction") == getElementData(vehicle, "faction")) then
		...
		if not isLocked or inCar then --if vehicle is not locked or if player is inside vehicle
			...
			if exports['cr_items']:hasItem(vehicle, 117) then
	...
	if vx < 0.05 and vy < 0.05 and vz < 0.05 and not getPedOccupiedVehicle(localPlayer) and not isVehicleLocked(vehicle) then -- completely stopped
		...
		if trailers[getElementModel(vehicle)] then
			if exports.cr_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then
				row.park = rightclick:addRow("Park")
				addEventHandler("onClientGUIClick", row.park, parkTrailer, false)
			end
		else
			if exports.cr_global:hasItem(localPlayer, 57) then -- FUEL CAN
	...
	--admin stuff (Exciter)
	if (exports.cr_integration:isPlayerTrialAdmin(localPlayer) or exports.cr_integration:isPlayerRehber(localPlayer) or exports.cr_integration:isPlayerScripter(localPlayer)) then
		if (exports.cr_global:isStaffOnDuty(localPlayer) or exports.cr_integration:isPlayerScripter(localPlayer)) then
			row.respawn = rightclick:addRow("ADM: Respawn")
			addEventHandler("onClientGUIClick", row.respawn, fRespawn, false)

			if (exports.cr_integration:isPlayerSeniorAdmin(localPlayer) or exports.cr_integration:isPlayerScripter(localPlayer)) then
...
function lockUnlock(button, state)
	if (button=="left") then
		...
		elseif exports.cr_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (getElementData(localPlayer, "faction") > 0 and getElementData(localPlayer, "faction") == getElementData(vehicle, "faction")) then
...
function fStretcher(button, state)
	if (button=="left") then
		if not (isVehicleLocked(vehicle)) then
			triggerServerEvent("stretcher:createStretcher", localPlayer, false, vehicle)
		end
	end
end
...
function fLook(button, state)
	if (button=="left") then
		triggerEvent("editdescription", localPlayer)
	end
end
...
function fillFuelTank(button, state)
	if (button=="left") then
		local _,_, value = exports.cr_global:hasItem(localPlayer, 57)
		if value > 0 then
...
function knockVehicle()
	triggerServerEvent("onVehicleKnocking", localPlayer, vehicle)
end
...
function fRespawn()
	triggerServerEvent("vehicle-manager:respawn", localPlayer, vehicle)
end
