...
function konus(element)
	...
	if (ped=="Steven Pullman") then
		...
		if (getElementData(element, "activeConvo")~=1) then
			triggerEvent ("stevieIntroEvent", localPlayer)
		end
	...
	elseif (ped=="Emma Brewles") then -- Emma Brewles
		triggerEvent("onLicense", localPlayer)
	elseif (ped=="Aygun Nagiyeva") then -- 
		triggerEvent("advert.advertGUI", localPlayer)
	...
	elseif (ped=="Aaron Thompson") then
		triggerServerEvent("papazAmca", localPlayer)
	elseif (ped=="Kıyafet Mağazası") then
        triggerEvent("boutique.renderUI", localPlayer)
	elseif (ped== "Aksesuar Mağazası") then
		triggerEvent("onWearableShop", localPlayer)
	else
		triggerEvent("shop.open", localPlayer, element)
	end
end
addEvent("shop.talkPed",true)
addEventHandler("shop.talkPed", root, konus)
...
local fonts = {
	fontBold = exports.cr_fonts:getFont("sf-bold", 15),
	fontRegular = exports.cr_fonts:getFont("sf-regular", 8),
}
	...
	if getElementData(localPlayer, "loggedin") == 1 then
		for _, ped in ipairs(getElementsByType("ped")) do
			...
			if pedName and pedTalkStatus == 1 then
				local playerX, playerY, playerZ = getPedBonePosition(localPlayer, 8)
				local pedX, pedY, pedZ = getPedBonePosition(ped, 1)

				if getDistanceBetweenPoints3D(playerX, playerY, playerZ, pedX, pedY, pedZ) < 1.1 then
					local screenX, screenY = getScreenFromWorldPosition(pedX, pedY, pedZ + 0.3)

					if screenX and screenY then
						dxDrawRectangle(screenX - 50 / 2, screenY + 20, 35, 35, tocolor(10, 10, 10, 250))
						dxDrawRectangle(screenX - 50 / 2, screenY + 52, progress, 3, exports.cr_ui:getServerColor(1, 250))
						dxDrawText("E", screenX - 78 / 2, screenY + 26, screenX - 50 / 2 + 50, screenY + 50, exports.cr_ui:getServerColor(1, 250), 1, fonts.fontBold, "center", "center")
						dxDrawText(pedName:gsub("_", " "), screenX - 78 / 2, screenY + 80, screenX - 50 / 2 + 50, screenY + 50, tocolor(255, 255, 255, 250), 1, fonts.fontRegular, "center", "center")
						dxDrawText("Konuşmak için basılı tutun.", screenX - 78 / 2, screenY + 110, screenX - 50 / 2 + 50, screenY + 50, tocolor(255, 255, 255, 250), 1, fonts.fontRegular, "center", "center")
						...
						if not isCursorShowing() then
							if getKeyState("E") then
								if progress ~= 35 then
									...
									triggerEvent("shop.talkPed", localPlayer, ped)
									progress = 0
								end
							else
								progress = 0
