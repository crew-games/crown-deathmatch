...
setTimer(function()
    if getElementData(localPlayer, "loggedin") == 1 then
	    if getElementData(localPlayer, "hud_settings").radar ~= 2 or getElementInterior(localPlayer) ~= 0 or (getElementDimension(localPlayer) ~= 0 and getElementDimension(localPlayer) ~= 1) then 
	        setPlayerHudComponentVisible("radar", false)
	    else 
