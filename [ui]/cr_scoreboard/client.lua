...
local function setupPlayersCache()
    ...
    for _, player_ in ipairs(getElementsByType("player")) do
        ...
        playersCache[tableInsertID] = {
            ...
            rank = exports.cr_rank:getPlayerRankTitle(player_),
            kills = player_:getData("kills"),
            deaths = player_:getData("deaths"),
...
local function showScoreboard()
    if localPlayer:getData("loggedin") == 1 then
        if not checkRender("renderScoreboard") then
            ...
            if not checkRender("renderScoreboard") then
                createRender("renderScoreboard", renderScoreboard, 0)
            end
        end
    end
end
...
addEventHandler("onClientPlayerJoin", root, function()
    ...
    playersCache[tableInsertID] = {
		...
		rank = exports.cr_rank:getPlayerRankTitle(source),
        kills = source:getData("kills"),
        deaths = source:getData("deaths"),
...
function renderScoreboard()
    ...
    renderColumn({
        ...
        x = width,
        y = rowHeight
    }, { 'ID', 'Karakter Adı', 'Rütbe', 'Öldürme', 'Ölme', 'Seviye', 'Ping' }, tocolor(18, 18, 20), tocolor(102, 102, 102), fonts.columnRegular)
    ...
    for index, data in ipairs(playersCache) do
        if index >= detailScroll and index <= detailScrollMax then
            ...
            local name = playerStaff and data.name .. " (" .. (data.username or "?") .. ")" or data.name
            ...
            renderColumn({
                ...
                x = width,
                y = rowHeight
            }, { id, name, rank, kills, deaths, level, ping }, background, tocolor(r, g, b, alpha), fonts.body)
    ...
    dxDrawText('best of all time', panelX + 20, lastY + rowHeight, width + (panelX + 20), 35 + (lastY + rowHeight), tocolor(50, 50, 56, 205), 1, fonts.columnRegular, "left", "center", false, false, false, true)
