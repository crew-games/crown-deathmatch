...
function frames_showTextureSelection (slot, textureURL, imgData)
	local integration = exports.cr_integration
	local global = exports.cr_global
	local dimension = getElementDimension (localPlayer)
		
	if dimension >= 1 or (integration:isPlayerSeniorAdmin(localPlayer) and global:isAdminOnDuty(localPlayer)) or integration:isPlayerScripter(localPlayer) then
		if exports.cr_global:hasItem (localPlayer, 4, dimension) or exports.cr_global:hasItem (localPlayer, 5, dimension) or (integration:isPlayerAdmin1(localPlayer) and global:isAdminOnDuty(localPlayer)) or (dimension==0) then
...
function frames_loadClientInteriorTexture (imgData, textureName, interior)
	...
	for i,v in ipairs (worldShaders[interior].textures) do
		...
		if width > 1000 or height > 1000 then
			if exports.cr_global:hasItem (localPlayer, 4, interior) or exports.cr_global:hasItem (localPlayer, 5, interior) then
