...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 215, 202, "Crown Deathmatch - Silah Konum", false)
...
addCommandHandler("silahkonum", createSelectionWindow)
	...
	if id then
		if not getElementData(localPlayer, "attachingWeapon" .. id) then
			...
			if ZORUNLU_OLMAYAN[id] then
				...
				outputChatBox("[!]#FFFFFF Bu silahın görünmesi zorunludur!",255,0,0, true)
			end
		end
		...
		addEventHandler("onClientGUIChanged", posEditBox[i],
			function()
				...
				if math.abs(x) > 0.5 or math.abs(y) > 0.5 or math.abs(z) > 0.5 then
					...
					if isElement(editingObject) then
						exports.cr_bones:attachElementToBone(editingObject, localPlayer, bone, x, y, z, rx, ry, rz)
					end
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	exports.cr_bones:attachElementToBone(object, localPlayer, bone, x, y, z, rx, ry, rz)
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		if isElement(editingObject) then
			exports.cr_bones:attachElementToBone(editingObject,
		...
		outputChatBox("[!]#FFFFFF " .. getWeaponNameFromID(editingWeapon) .. " markalı silahın pozisyonu " .. editingSkin .. " ID'li skin için uygulandı.", 0, 255, 0, true)
	end
end
...
function resetWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		outputChatBox("[!]#FFFFFF " .. getWeaponNameFromID(editingWeapon) .. " markalı silahın pozisyonu " .. editingSkin .. " ID'li skin için sıfırlandı.", 0, 255, 0, true)
	end
end
