...
-- Server-Side
-- Created by Exciter for Crown Deathmatch, 15.05.2014 (DD/MM/YYYY)
...
function addArtifact(player, artifact, noOutput, customItemTexture) --Start to wear an artifact the player is not already wearing
	if player and artifact then	
		if artifacts[player] and artifacts[player][artifact] then
			...
			exports.cr_bones:attachElementToBone(object,player,data[ART_BONE],data[ART_X],data[ART_Y],data[ART_Z],data[ART_RX],data[ART_RY],data[ART_RZ])
			...
			if data[ART_TEXTURE] then
				...
				--triggerClientEvent(root, "artifacts:addTexture", player, object, data[ART_TEXTURE])
				for k,v in ipairs(data[ART_TEXTURE]) do
					exports["cr_item-texture"]:addTexture(object, v[2], v[1])
				end
			...
			if not noOutput and g_artifacts_mes[artifact] and g_artifacts_mes[artifact][1] then
				exports.cr_global:sendLocalMeAction(player, tostring(g_artifacts_mes[artifact][1]))
			end
		end
	end
end
	...
	if player and artifact then	
		if not artifacts[player] or not artifacts[player][artifact] then
			...
			if not noOutput and g_artifacts_mes[artifact] and g_artifacts_mes[artifact][2] then
				exports.cr_global:sendLocalMeAction(player, tostring(g_artifacts_mes[artifact][2]))
			end
		end
	end
end
...
function setPlayerArtifactProperty(player, artifact, property, value) --exported function to let other scripts change things about a worn artifact (adjust position, change model, change bone, etc.)
	if artifacts[player] and artifacts[player][artifact] and isElement(artifacts[player][artifact]) then
		...
		elseif (property == "bone") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,value,x,y,z,rx,ry,rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "x") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,(x+value),y,z,rx,ry,rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "y") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,x,(y+value),z,rx,ry,rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "z") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,x,y,(z+value),rx,ry,rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "rx") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,(rx+value),ry,rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "ry") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,(ry+value),rz)
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif (property == "rz") then
			local ped, bone, x, y, z, rx, ry, rz = exports.cr_bones:getElementBoneAttachmentDetails(object)
			exports.cr_bones:detachElementFromBone(object)
			local result = exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,(rz+value))
			if not result then
				exports.cr_bones:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
