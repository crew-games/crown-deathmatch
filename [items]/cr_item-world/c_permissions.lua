...
function itemPropertiesGUI(object)
	...
	if canEditItemProperties(localPlayer, object) then
		...
		local itemName = tostring(exports.cr_global:getItemName(itemID))
		...
		if newCreatorName then
			...
			triggerServerEvent("item-world:getItemPropertiesData", resourceRoot, object)
		end
	end
end
addEvent("showItemProperties", true)
addEventHandler("showItemProperties", root, itemPropertiesGUI)
...
addEventHandler("item-world:fillItemPropertiesGUI", resourceRoot, itemPropertiesGUIFillData)
...
function saveItemProperties(button, state)	
	...
	triggerServerEvent("item-world:saveItemProperties", resourceRoot, object, use, useData, move, moveData, pickup, pickupData)
