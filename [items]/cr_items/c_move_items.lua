...
local function forceUpdate()
	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			if interiorType==2 then
				if (not exports.cr_faction:isInFactionType(localPlayer, 2) and not exports.cr_faction:isInFactionType(localPlayer, 3) and getElementData(localPlayer, "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
	...
	if getElementType(element) == "vehicle" then
		...
		if exports.cr_integration:isPlayerTrialAdmin(localPlayer) then
...
function hasKey(key, element)
	...
	elseif getElementData(localPlayer, "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.cr_faction:isInFactionType(localPlayer, 2) and not exports.cr_faction:isInFactionType(localPlayer, 3)) then
				return false, false
			else
				return true, true
			end
...
local function moveFromElement(button)
	...
	if button == "left" and col ~= -1 and row ~= -1 then
		...
		if slot < 0 --[[and exports.cr_integration:isPlayerTrialAdmin(localPlayer)]] then
			...
			if item then
				...
				if (hasKey(getElementData(element, "dbid"), element) and getElementType(element)=="interior") or getElementType(element)~="interior" then
					if itemID < 0 and itemID ~= -100 then -- weapon
						local free, totalfree = exports.cr_weaponcap:getFreeAmmo(-itemID)
						local cap = exports.cr_weaponcap:getAmmoCap(-itemID)
						if totalfree == 0 then
					...
					outputChatBox("You must have the keys to his house to take items from the Mailbox.", localPlayer, 255, 0, 0)
				end
			end
		end
	end
...
local function openElementInventory(ax, ay)
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		exports.cr_global:centerWindow(wInventory)
		
		lYou = guiCreateLabel(0.05, 0.1, 0.45, 0.05, "Sen", true, wInventory)
		guiSetFont(lYou, "default-bold-small")
		guiLabelSetHorizontalAlign(lYou, "center")

		lVehicle = guiCreateLabel(0.5, 0.1, 0.45, 0.05, "Hedef", true, wInventory)
		guiSetFont(lVehicle, "default-bold-small")
		guiLabelSetHorizontalAlign(lVehicle, "center")

		gUserItems = guiCreateGridList(0.05, 0.15, 0.45, 0.73, true, wInventory)
		UIColName = guiGridListAddColumn(gUserItems, "Eşya", 0.9)

		gElementItems = guiCreateGridList(0.5, 0.15, 0.45, 0.73, true, wInventory)
		VIColName = guiGridListAddColumn(gElementItems, "Eşya", 0.9)

		bCloseInventory = guiCreateButton(0.05, 0.9, 0.9, 0.075, "Envanteri Kapat", true, wInventory)
...
addEventHandler("onClientChangeChar", root, hideMenu)

addEvent("finishItemMove", true)
addEventHandler("finishItemMove", localPlayer,
	function()
...
function collisionFix()
	for key, value in pairs(getElementsByType("object", getResourceRootElement(getResourceFromName("cr_item-world")))) do
