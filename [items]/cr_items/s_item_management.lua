		...
		if result then
			...
			repeat
				...
				if row then
					count = count + 1
					--print(row.itemID)
					saveditems[element][count] = { tonumber(row.itemID), tonumber(row.itemValue) or row.itemValue, tonumber(row.index), tonumber(row.protected) }
				end
			until not row
							...
							if query then
								...
								if ((itemID >= 31) and (itemID <= 43)) or itemBannedByAltAltChecker[itemID] then
									if itemID == 150 then
										if getElementModel(from) == 2942 or getElementModel(to) == 2942 then
											...
											triggerClientEvent(to,"chef_item",to,getItemName(itemID,itemValue),itemID,"Eklendi")
											return true
										end
									end
								end
								...
								if itemID == 134 then -- MONEY
									if takeItemFromSlot(from, slot, true) then
										if exports.cr_global:giveMoney(to, tonumber(itemValue)) then
											return true
										end
									end
								else
...
function deleteAll(itemID, itemValue)
	if itemID then
		...
		if saveditems then
			for value in pairs(saveditems) do
				if isElement(value) then
					while exports.cr_global:hasItem(value, itemID, itemValue) do
						exports.cr_global:takeItem(value, itemID, itemValue)
					end
				end
			end
		end
		...
		-- remove world items
		local objects = getElementsByType('object', getResourceRootElement(getResourceFromName('cr_cr_cr_item-world')))
		if objects then
...
-- DELETE ALL ITEMS WITHIN AN INT - Farid
...
function fixInventory(thePlayer, commandName)
	if not isTimer(fixInventoryTimer[thePlayer]) then
		if (not getElementData(thePlayer, "baygin")) and (not getElementData(thePlayer, "protection")) and (not getElementData(thePlayer, "adminjailed")) then
