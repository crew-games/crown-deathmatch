...
function canPickup(player, item)
	if isProtected(player, item) then
		...
		if isElement(item) then
			if exports['cr_item-world']:can(player, "pickup", item) then
				return true
			else
				return false
			end
...
function canMove(player, item)
	if isProtected(player, item) then
		...
		if isElement(item) then
			if exports['cr_item-world']:can(player, "move", item) then
				return true
			else
				return false
			end
...
function protectItem(faction, item, slot)
	--[[if getElementParent(getElementParent(source)) ~= getResourceRootElement(getResourceFromName("cr_item-world")) then
		return
	end]]
	if getElementData(source, "itemID") then
		local itemID = getElementData(source, "itemID")
		local index = getElementData(source, "id")
	    if itemID == 169 then --disable for keypadlock / Farid
	      	return false
	    end
		
		if type(faction) == "number" and exports.cr_global:isStaffOnDuty(client) then
...
function SmallestID() -- finds the smallest ID in the SQL instead of auto increment / Farid
...
addEventHandler("dropItemOnDead", root, dropItemOnDead)
...
function dropItem(itemID, x, y, z, ammo, keepammo)
	...
	if not ammo then
		...
		if (itemID == 115) then -- Weapons
			local itemCheckExplode = exports.cr_global:explode(":", itemValue)
			-- itemCheckExplode: [1] = gta weapon id, [2] = serial number, [3] = weapon name
			local weaponDetails = exports.cr_global:retrieveWeaponDetails(itemCheckExplode[2] )
		...
		elseif (itemID == 116) then -- Ammo
			local ammoDetails = exports.cr_global:explode(":", itemValue )
		...
		if itemID == 60 or itemID == 137 or itemID == 138 or itemID == 134 or itemID == 115 or itemID == 116 or itemID == 175 and not exports.cr_integration:isPlayerAdmin1(source) then
			outputChatBox("[!]#FFFFFF Bu nesneyi yere bırakamazsın.", source, 255, 0, 0, true)
		...
		elseif itemID == 139 then
			...
			--[[if not (hasItem(source, itemID)) then
				...
				exports.cr_logs:logMessage(haxStr, 32)
				exports.cr_global:sendMessageToAdmins("AdmWrn: " .. haxStr)		
			end]]
			...
			elseif itemID == 134 then
				if not exports.cr_global:takeMoney(source, itemValue) then
					outputDebugString(getPlayerName(source) .. ' (' .. getPlayerSerial(source) .. ') tried dropping money he doesnt have')

					while exports['cr_items']:takeItem(source, 134) do
					end

					local amount = exports.cr_global:getMoney(source)
					if amount > 0 then
						exports.cr_global:giveItem(source, 134, amount)
					end
			...
			elseif (itemID == 147) then -- Picture Frame
				local dimension = getElementDimension(source)
				if dimension > 0 or (exports.cr_integration:isPlayerSeniorAdmin(source) and exports.cr_global:isAdminOnDuty(source)) or exports.cr_integration:isPlayerScripter(source) then
					local split = exports.cr_global:explode(";", itemValue)
					local url = split[1]
					local texture = split[2]
					if url and texture then
						if exports['cr_texture']:newTexture(source, url, texture) then
			...
			elseif itemID == 169 then --Keypad Door Lock / Farid
				local maxRange = 10
				...
				--check if int is existed and int is owned by dropper / Farid
				...
				if not validInterior then
					--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "The interior that this device is made for is no longer existed. Thus far this device is not usable anymore.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end

				if not validDropper then
					--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "Interior '" .. interiorName .. "' is no longer owned by you. Thus far this device is not usable anymore.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end

				--check if the interior is unlocked / Farid
				if isIntLocked then
					--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "Interior '" .. interiorName .. "' is locked, please unlock it first.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end

				-- Making sure the interior door is valid / Farid
				if not doorOutside or not doorInside then
					--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "Interior '" .. interiorName .. "' is defected, please notify scripters.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end

				-- check if the devide is placed outside within maxRange / Farid
				if interior == doorOutside[4] and dimension == doorOutside[5] then
					if getDistanceBetweenPoints3D(x,y,z,doorOutside[1], doorOutside[2], doorOutside[3]) > maxRange then
						--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "This device can only be installed within " .. math.floor(maxRange/2) .. " meters from its out-door.")
						triggerClientEvent(source, "finishItemDrop", source)
						return false
					end
				-- check if the devide is placed inside within maxRange / Farid
				elseif interior == doorInside[4] and dimension == tonumber(itemValue) then
					if getDistanceBetweenPoints3D(x,y,z,doorInside[1], doorInside[2], doorInside[3]) > maxRange then
						--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "This device can only be installed within " .. math.floor(maxRange/2) .. " meters from its in-door.")
						triggerClientEvent(source, "finishItemDrop", source)
						return false
					end
				else
					--exports.cr_hud:sendBottomNotification(source, getItemName(itemID, itemValue), "This device can only be installed within " .. math.floor(maxRange/2) .. " meters from its door.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end
			end
			
			local smallestID = SmallestID() -- Farid
			...
			if insert then
				...
				-- Animation
				exports.cr_global:applyAnimation(source, "CARRY", "putdwn", 500, false, false, true)
				...
				local obj = exports['cr_item-world']:createItem(id, itemID, itemValue, modelid, x, y, z + zoffset - 0.05, rx, ry, rz+rz2)
				...
				if objTexture then
					for objTexKey, objTexVal in ipairs(objTexture) do
						exports['cr_item-texture']:addTexture(obj, objTexVal[2], objTexVal[1])
					end
				end
				...
				--exports.cr_logs:logMessage(getElementData(source, "account:username") .. "\\" .. getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName(itemID) .. " - " .. itemValue, 17)
				exports.cr_logs:dbLog(source, 39, source, getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName(itemID) .. " - " .. itemValue)
				...
				if itemID == 169 then --Keypad Door Lock / Farid
					triggerEvent("installKeypad", source, obj, keypadDoorInterior)
				end
				
				--SEND ME FOR WHAT HAS BEEN DROPPED
				if itemID == 134 then
					triggerEvent('sendAme', source, "yere $" .. exports.cr_global:formatMoney(itemValue) .. " bırakır.")
		...
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			...
			--[[local totalAmmoPosessed = exports.cr_global:getWeaponCount(source, itemID) 
			...
			if query then
				...
				exports.cr_global:takeWeapon(source, itemID)
				if keepammo then
					exports.cr_global:giveWeapon(source, itemID, keepammo)
				end
				...
				local obj = exports['cr_item-world']:createItem(id, -itemID, ammo, modelid, x, y, z - 0.4, 75, -10, rz2)
				exports.cr_pool:allocateElement(obj)
				...
				--exports.cr_logs:logMessage(getElementData(source, "account:username") .. "\\" .. getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName(-itemID) .. " - " .. ammo, 17)
				exports.cr_logs:dbLog(source, 39, source, getPlayerName(source).. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName(-itemID) .. " - " .. ammo)
			end
		end
	end
...
function doItemGiveawayChecks(player, itemID, itemValue)
	...
	--if itemID == 16 and not hasItem(source, 16, getPedSkin(source)) then
		...
		setElementData(source, 'clothing:id', nil, true)
		dbExec(exports.cr_mysql:getConnection(), "UPDATE characters SET skin = '" .. tostring(getElementModel(source)) .. "', clothingid = NULL WHERE id = '" .. tostring(getElementData(source, "dbid")) .. "'")
	end
	...
	if badge and getElementData(source, badge[1]) and not hasItem(source, itemID, removeOOC(getElementData(source, badge[1]))) then
		triggerEvent('sendAme', source, badge[2] .. " siler.")
		setElementData(source, badge[1], nil)
...
local function moveItem(item, x, y, z)
	...
	--ANTI ALT-ALT / Farid
	if not exports.cr_global:isStaffOnDuty(source) then
	...
	if not (z) then
		destroyElement(item)
		exports.cr_mysql:query_free("DELETE FROM worlditems WHERE id = " .. exports.cr_mysql:escape_string(id))
		return
	end
	...
	if not exports.cr_integration:isPlayerTrialAdmin(source) and (itemID == 81 or itemID == 103) then
		return
	end
	
	if itemID == 138 then
		if not exports.cr_integration:isPlayerAdmin1(source) then
			outputChatBox("Only a Lead+ admin can move this item.", source, 255, 0, 0)
			return
		end
	end
	
	if itemID == 139 and not exports.cr_integration:isPlayerTrialAdmin(source) then
...
addEventHandler('item:move:save', root,
	function(x, y, z, rx, ry, rz)
		...
		if exports.cr_global:isStaffOnDuty(client) or exports.cr_integration:isPlayerTrialAdmin(client) or exports.cr_integration:isPlayerScripter(client) then
			...
			local itemID = getElementData(source, "itemID")
			if itemID == 138 and not exports.cr_integration:isPlayerAdmin1(client) then
				outputChatBox("Only a Lead+ admin can move this item.", client, 255, 0, 0)
				reset()
				return
			end
			
			if itemID == 139 and not exports.cr_integration:isPlayerTrialAdmin(client) then
...
local function rotateItem(item, rz)
	...
	rz = rz + rzx
	local result = mysql:query_free("UPDATE worlditems SET rz = " .. rz .. " WHERE id = " .. exports.cr_mysql:escape_string(id))
	if result then
...
function pickupItem(object, leftammo)
	...
	setElementData(object, "transfering", true, "none") --No sync at all.
	...
	if not canPickup(source, object) then
		...
		setElementData(object, "transfering", nil)
		return
	end
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.cr_global:isStaffOnDuty(source) then
		...
		if creator ~= picker then
			local accountCreator = exports.cr_cache:getAccountFromCharacterId(creator)
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				...
				exports.cr_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Detected illegal assets transferring on account name '" .. getElementData(source, "account:username") .. "'.")
				exports.cr_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Suspect is trying to transfer a " .. getItemName(itemID) .. " between alternatives.")
				exports.cr_logs:dbLog(source, 5, {object}, "TRIED TO ALT-ALT " .. getItemName(itemID) .. " between alternatives.")
				setElementData(object, "transfering", nil)
				return false
			end
		end
	...
	--[[This block is currently doing nothing but preventing you from picking up generic item so I disabled it./ Farid
	if (itemID==80) then
	...
	exports.cr_global:applyAnimation(source, "CARRY", "liftup", 600, false, true, true)
	if itemID > 0 then
		mysql:query_free("DELETE FROM worlditems WHERE id='" .. id .. "'")
		...
		if itemID == -100 then
			...
			exports.cr_global:giveWeapon(source, -itemID, itemValue, true)
		end
	end

	exports.cr_logs:dbLog(source, 39, {object}, "~World #" ..getElementID(object) .. "->" .. getPlayerName(source) .. " - " ..getItemName(itemID).. " - " ..itemValue)
	destroyElement(object)
	
	if itemID == 134 then -- money
		exports.cr_global:giveMoney(source, itemValue)
		outputChatBox("You picked up $" .. exports.cr_global:formatMoney(itemValue) .. ".", source, 255, 194, 14)
		triggerEvent('sendAme', source, "bends over and picks up $" .. exports.cr_global:formatMoney(itemValue) .. ".")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
...
function removeItemTransferingState()
	for i, object in pairs(exports.cr_pool:getPoolElementsByType("object")) do
		setElementData(object, "transfering", nil)
	end
end
