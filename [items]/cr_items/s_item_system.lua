...
function removeOOC(text)
	if not text then
		outputChatBox("Error code 0001 occurs, please report to Crown Deathmatch MTA Scripting Team at http://forums.crowndeathmatch.net/forms.php?do=form&fid=1")
		return ""
	end
...
function useItem(itemSlot, additional)
	...
	if itemID then
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			exports.cr_bones:attachElementToBone(bottleObj,source,11,0,0,0,0,0,0)
			thePlayer = source
			setTimer(function()
		...
		elseif (itemID==226) then
			if not (getElementData(source, "Atatürk Bayrağı")) then
				...
				--exports.cr_bones:attachElementToBone(bayrak, source, 11, 0, 0, 0, 0, 180, 100)
				exports.cr_bones:attachElementToBone(bayrak, source, 12, 0, 0, 0, 0, 200, 30)
				...
				exports.cr_bones:detachElementFromBone(bayrak)
