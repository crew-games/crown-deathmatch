...
local function updateVolume(blaster)
    ...
    if sound then
        ...
        if isPedInVehicle(localPlayer) and getElementData(getPedOccupiedVehicle(localPlayer), 'vehicle:windowstat') ~= 1  then
            volume = 0.3 * volume
        end
...
function startGB(blaster)
    ...
    if station > 0 and volume > 0 then
        local streams = exports.cr_radio:getStreams()
    ...
    function ()
        for i,v in pairs (blasters) do
            ...
            if sx and distance <= 10 then
                ...
                local streams = exports.cr_radio:getStreams()
                exports.cr_ui:dxDrawFramedText("#" .. itemValue .. " - " .. streams[itemValue][1], sx, sy, sx, sy, tocolor(255, 255, 255, 255), 0.85, "default-bold", "center")
                
                if type(song) == "string" then
                    exports.cr_ui:dxDrawFramedText("Oynuyor: " .. song, sx, sy + 15, sx, sy, tocolor(255, 255, 255, 255), 0.85, "default-bold", "center")
                end
            end
        end
    end)
