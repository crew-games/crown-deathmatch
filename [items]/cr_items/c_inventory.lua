-- Configuration
local background_color = tocolor(10, 10, 10, 245)
...
local actionIcons =
{
	[TAB_WALLET] = { "wallet", tocolor(255, 255, 255, 50), "Cüzdan" },
	[TAB_ITEMS] = { "backpack", tocolor(255, 255, 255, 50), "Envanter" },
	[TAB_KEYS] = { "keys", tocolor(255, 255, 255, 50), "Anahtarlar" },
	[TAB_WEAPONS] = { "weapons", tocolor(255, 255, 255, 50), "Silahlar" },
	
	[ACTION_DROP] = { "drop", tocolor(127, 255, 127, 63), "Eşyayı Bırak", "Otomatik olarak bırakmak için bir öğeyi seçerken CTRL tuşuna basın." },
	[ACTION_SHOW] = { "show", tocolor(127, 127, 255, 63), "Eşyayı Göster" },
	[ACTION_DESTROY] = { "delete", tocolor(255, 127, 127, 63), "Eşyayı Sil", "Otomatik olarak silmek için bir öğeyi seçerken DELETE tuşuna basın." },
}
...
local function getHoverElement(force)
	...
	for _, acceptProtected in ipairs({false, true}) do
		...
		if element and getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("item_world")) then
			return element
		elseif b and c and d then
			element = nil
			local x, y, z = nil
			local maxdist = 0.34
			for key, value in ipairs(getElementsByType("object",getResourceRootElement(getResourceFromName("item_world")), true)) do
...
local function tooltip(x, y, text, text2, to_delete)
	...
	dxDrawRectangle(x-(width/2), y, width, height, tocolor(10, 10, 10, 245), true)
...
local function getImage(itemID, itemValue)
	if not itemID or not tonumber(itemID) then --Stop bugging out int when picking up items too fast / sildev
		return "images/yok.png"
	end
	...
	elseif itemID == 49 then
		return "images/" .. (itemValue== 1 and "49" or "49b") .. ".png"
	...
	elseif itemID == 165 then -- Animated Texture
		...
		if fileExists("images/" .. itemID .. ".png") then -- düstüm ya
...
local function getOverlayText(itemID, itemValue, isGrouped)
	...
	elseif itemID == 116 then
		...
		text = text .. s[3]:gsub("Mermisi ", "") .. " Mermisi"
	elseif itemID == 134 then -- Money / sildev
		text = "$" .. exports.cr_global:formatMoney(itemValue)
	elseif itemID == 150 then -- ATM Card / sildev
		text = g_items[itemID][1]
	elseif itemID == 152 then -- ID Card / sildev
		text = "Kimlik Kartı"
	...
	elseif itemID == 154 then
		text = "Balıkçılık Lisansı"
	elseif itemID == 78 then
		text = "Pilot Sertifikası"
	end
...
local function getTooltipFor(itemID, itemValue, isGrouped, isProtected)
	...
	if isGrouped then
		return isGrouped .. "x " .. name, "Hepsini görmek için tıkla."
	end
	...
	if itemID == 134 then
		if itemValue and itemValue ~= 1 and #tostring(itemValue) > 0 and not g_items[itemID][2]:find("#v") and itemValue ~= itemName then -- if #v is there, it's put into the description already
			name = name .. " ($" .. exports.cr_global:formatMoney(itemValue) .. ")"
		end
	...
	function()
		...
		elseif not guiGetInputEnabled() and not isMTAWindowActive() and isCursorShowing() then
			...
			if show then
				...
				local y2 = y + sbox
				dxDrawRectangle(x-80, y, sbox, ((jrows - irows + 1) * sbox + spacer)*1.25, background_color)
				...
				for i = irows, jrows do
					...
					dxDrawImage(boxx-9, boxy+5, 50, 50, "images/category/" .. icon[1] .. ".png", 0, 0, 0, i == activeTab and exports.cr_ui:getServerColor(1) or tocolor(200, 200, 200, 225))
					
					if not clickWorldItem and inArea(cursorX, cursorY, boxx, boxx + box, boxy, boxy + box) then
						if i <= 6 then
				...
				for i = 1, columns * 5 do
					...
					color = i % 2 == 1 and tocolor(20, 20, 20, 220) or tocolor(33, 33, 33, 210)
					
					local item = inventory[i]
					if item then
						if not isMove or item[4] ~= clickItemSlot.id then
							dxDrawRectangle(boxx, boxy, box, box, color)
							dxDrawImage(boxx+12, boxy+13, box-25, box-25, getImage(item[1], item[2]))
							
							-- overlay text for some items
							local text = getOverlayText(item[1], item[2], item[5])
							if #text > 0 then
								dxDrawText(text, boxx + 2, boxy + 2, boxx + box - 2, boxy + box - 2, tooltip_text_color, 1, fonts[1], "right", "bottom", true, true, true)
							end
							
							if not isMove and not clickWorldItem and inArea(cursorX, cursorY, boxx, boxx + box, boxy, boxy + box) then
						...
						dxDrawText("BOŞ", boxx, boxy, boxx+box, boxy+box, tocolor(255, 255, 255, 245), 1, font, "center", "center")
						dxDrawRectangle(boxx, boxy, box, box, color)
					end
				end
			end
			...
			if clickDown and (getTickCount() - clickDown >= 200) and ((clickItemSlot and not clickItemSlot.group) or (clickWorldItem and isElement(clickWorldItem) and not getElementData(clickWorldItem, "protected"))) then
				...
				if clickWorldItem then
					...
					if isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementModel(hoverElement) == 2942 and item [1] == 150 then -- ATM card into ATM/ sildev
								color = background_movetoelement_color
							...
							elseif getElementModel(hoverElement) == 3761 then -- shelf - sildev
					...
					if not isCursorOverInventory then
						...
						elseif hoverElement then
							...
							--[[elseif getElementType(hoverElement) == "ped"  and  getElementData(hoverElement,"shopkeeper") and item [1] == 121 then --and getElementData(hoverElement,"shopkeeper") then --  item [1] == 121
								color = background_movetoelement_color]] -- Disabled/ sildev
								
							elseif getElementType(hoverElement) == "ped" and  getElementData(hoverElement,"customshop") then --  import products to custom shop
								color = background_movetoelement_color
							elseif getElementParent(getElementParent(hoverElement)) ~= getResourceRootElement(getResourceFromName("item_world")) or hoverElement == getHoverElement() then
								color = full_color
							elseif tonumber(getElementData(hoverElement, "itemID")) == 166 and item[1] == 165 then -- Video disc into video player /Exciter
								color = background_movetoelement_color
							else
				...
				dxDrawRectangle(boxx - spacer, boxy - spacer, box + 2 * spacer, box + 2 * spacer, tocolor(200, 200, 200, 50))
				...
				if hoverElement then
					if color == background_movetoelement_color then
						...
						elseif getElementModel (hoverElement) == 3761 then
							name = "Shelf"
						elseif getElementModel (hoverElement) == 2942 then -- ATM card into ATM/sildev
							name = "ATM"
						elseif hoverElementItemID == 166 then --Video System
							name = "Video Player"
						end
						tooltip(boxx + sbox, boxy + (box - 50) / 2, getItemName(item[1], item[2]), "Taşı: " .. name)
...
addEventHandler("onClientClick", root,
	function(button, state, cursorX, cursorY, worldX, worldY, worldZ)
		if not waitingForItemDrop then
			if button == "left" or (button == "middle" and exports.cr_integration:isPlayerTrialAdmin(localPlayer)) then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SHOW then
								...
								elseif item[1] == 150 then--ATM card
									...
									local owner = exports.cr_cache:getCharacterNameFromID(itemExploded[2])
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								if itemID == 48 and countItems(localPlayer, 48) == 1 then -- backpack
									if getCarriedWeight(localPlayer) - getItemWeight(48, 1) > 10 then
										...
										triggerServerEvent("destroyItem", localPlayer, itemSlot, item, itemID)
									end
								elseif itemID == 134 then --Money
									outputChatBox("#FFFFFFParayı silemezsin.",255,0,0,true)
								else
									triggerServerEvent("destroyItem", localPlayer, itemSlot, itemID)
								end
							elseif hoverAction == ACTION_SPLIT then
								outputChatBox("#FFFFFFBu sistem aktif değil.",localPlayer,0,255,0,true)
							end
						end
						hoverItemSlot = false
						clickItemSlot = false
				...
				elseif hoverWorldItem or clickWorldItem and isElement(clickWorldItem) then
						...
						if (getDistanceBetweenPoints3D(x, y, z, eX, eY, eZ)<=maxDistance) then						
							...
							--[[moved to object-interaction
								...
								if not getElementData (localPlayer, "exclusiveGUI") then
									triggerServerEvent("openFreakinInventory", localPlayer, hoverWorldItem, cursorX, cursorY)
								end
							--]]
							if itemID == 169 then
								if not getElementData (localPlayer, "exclusiveGUI") then
					...
					elseif state == "up" and clickWorldItem and isElement(clickWorldItem) then
						...
						if (getDistanceBetweenPoints3D(x, y, z, eX, eY, eZ)<=maxDistance) then	
							...
							if getTickCount() - clickDown < 200 then
								if canBePickedUp then
									if itemID == 169 then -- Keypad Door Lock / sildev
										if not getElementData(localPlayer, "exclusiveGUI") then
...
bindKey("i", "down",
	function()
		if getElementData(localPlayer, "loggedin") == 1 and not exports.cr_vs:isPlayerInGame(localPlayer) then
			if not getElementData(localPlayer, "player.Cuffed") then
				if show then
					hideNewInventory()
				elseif not getElementData(localPlayer, "adminjailed") and getElementData(localPlayer, "viewingInterior")~=1 and getElementData(localPlayer, "dead")~=1 and not getElementData(localPlayer, "kelepce") and not getElementData(localPlayer, "ipbagli") then
					...
					outputChatBox("[!]#FFFFFF Şuanda bu işlemi uygulayamazsınız.", 255, 0, 0 ,true)
				end
			end
		end
	end
...
function hideNewInventory()
	...
	if show then
		show = false
		showCursor(false)
		--exports["cr_realism"]:showSpeedo()
	end
end
addEvent("items:inventory:hideinv", true)
addEventHandler("items:inventory:hideinv", localPlayer, hideNewInventory)
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.cr_integration:isPlayerAdmin1(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.cr_integration:isPlayerAdmin1(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
...
local imgW, imgH = 70, 65
local boxW,boxH = 128,110
local boxX,boxY = (sx-boxW)/2,(sy-boxH)-50
local imgX,imgY = (sx-imgW)/2,boxY+22
local font = exports.cr_fonts:getFont('UbuntuRegular',10)

local animItems = {}
tetik = function(player, itemidx, itemvaluex,durumx,itemnamex)
    if player == localPlayer then 
		local name = itemnamex
		if itemvaluex ~= 1 then name = itemnamex .. " (" .. itemvaluex .. ")" end
		local nameW = dxGetTextWidth(name,1,font)
		table.insert(animItems,{
			start=getTickCount(),
			id=itemidx,
			name=name,
			value=itemvaluex,
			boxW = nameW>boxW and nameW+5 or boxW,
			boxX = nameW>boxW and (sx-nameW)/2 or boxX,
			durum=durumx and "Eklendi" or "Eksildi",
			image = getImage(itemidx, itemvaluex)
		})
    end
end
addEvent("item:anim", true)
addEventHandler("item:anim", root, tetik)

function isInventoryShow()
	return show
end