...
function doDrug4Effect()
	if not (drug4effect) then
		...
		addEventHandler("onClientRender", root, createRandomPeds)
	end
end
...
function resetDrug4Effect()
	...
	resetSkyGradient()
	setTimer(setWeather, 100, 1, weather)
	removeEventHandler("onClientRender", root, createRandomPeds)
...
function doDrug5Effect()
	if not (drug5effect) then
		...
		addEventHandler("onClientVehicleEnterDelayed", root, resetDrug5Effect)
		--setGameSpeed(1.5)
	end
end

function resetDrug5Effect(thePlayer)
	if not thePlayer or thePlayer == localPlayer then
		drug5effect = false
		--setGameSpeed(1)
		resetSkyGradient()
		setTimer(setWeather, 100, 1, weather)
		removeEventHandler("onClientVehicleEnterDelayed", root, resetDrug5Effect)
	end
end
...
local drugMessages = { "Ne oluyor amına koyim!", "Waow adamım, sen uçuyorsun!", "Dans etmek ister misin, orospu çocuğu?", "Polisleri arayayım." }
	...
	for key, value in ipairs(players) do
		if (value~=localPlayer) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<15) then
				...
				outputChatBox(charname .. ": " .. drugMessages[rand2], 255, 255, 255)
				break
			end
		end
	end
...
addEventHandler("onClientChangeChar", root, resetAllDrugs)
			...
			setTimer(
				function(source)
					if isPedInVehicle(localPlayer) then
						triggerEvent("onClientVehicleEnterDelayed", source, localPlayer)
					end
				end, 500, 1, source
			)
		end
...
function uyusturucuKrizi()
	local bagimlilikOrani = getElementData(localPlayer, "saglik:uyusturucu_bagimlilik") or 0
	local krizYok = not getElementData(localPlayer, "uyusturucu_kriz")
	if krizYok then
		return
	end
	if bagimlilikOrani <= 20 and bagimlilikOrani > 0 then
		setTimer(fadeCamera, 1000, 1, true)
		setTimer(fadeCamera, 2000, 1, false)
		setTimer(fadeCamera, 3000, 1, true)
		setTimer(fadeCamera, 4000, 1, false)
		setTimer(fadeCamera, 5000, 1, true)
		setTimer(fadeCamera, 6000, 1, false)
		setTimer(fadeCamera, 7000, 1, true)
		outputChatBox("** #f0f0f0Canın uyuşturucu almak istiyor. Gözlerin kapanıyor.", 255, 0, 0, true)
	elseif bagimlilikOrani <= 30 and bagimlilikOrani > 0 then
		setTimer(fadeCamera, 1000, 1, true)
		setTimer(fadeCamera, 2000, 1, false)
		setTimer(fadeCamera, 3000, 1, true)
		setTimer(fadeCamera, 4000, 1, false)
		...
		setTimer(fadeCamera, 6000, 1, false)
		setTimer(fadeCamera, 7000, 1, true)
		setTimer(fadeCamera, 8000, 1, false)
		setTimer(fadeCamera, 9000, 1, true)
		setTimer(fadeCamera, 10000, 1, false)
		setTimer(fadeCamera, 12000, 1, true)
	elseif bagimlilikOrani <= 50 and bagimlilikOrani > 0 then
		setTimer(fadeCamera, 1000, 1, true)
		setTimer(fadeCamera, 2000, 1, false)
		setTimer(fadeCamera, 3000, 1, true)
		setTimer(fadeCamera, 4000, 1, false)
		...
		setTimer(fadeCamera, 6000, 1, false)
		setTimer(fadeCamera, 7000, 1, true)
		setCameraDrunkLevel(250)
		setTimer(setCameraDrunkLevel, 5000, 1, 0)
		outputChatBox("** #f0f0f0Canın uyuşturucu almak istiyor. Gözlerin kapanıyor ve dengeni kaybediyorsun.", 255, 0, 0, true)
	elseif bagimlilikOrani <= 80 and bagimlilikOrani > 0 then
		...
		setTimer(fadeCamera, 12000, 1, true)
		setCameraDrunkLevel(250)
		setTimer(setCameraDrunkLevel, 10000, 1, 0)
		outputChatBox("** #f0f0f0Canın uyuşturucu almak istiyor. Gözlerin kapanıyor ve dengeni kaybediyorsun.", 255, 0, 0, true)
	elseif bagimlilikOrani <= 100 and bagimlilikOrani > 0 then
		setElementHealth(localPlayer, getElementHealth(localPlayer) - 10)
		setTimer(fadeCamera, 1000, 1, true)
		setTimer(fadeCamera, 2000, 1, false)
		setTimer(fadeCamera, 3000, 1, true)
		...
		setTimer(fadeCamera, 10000, 1, false)
		setTimer(fadeCamera, 12000, 1, true)
		setTimer(fadeCamera, 13000, 1, false)
		setTimer(fadeCamera, 14000, 1, true)
		setTimer(fadeCamera, 15000, 1, false)
		setTimer(fadeCamera, 17000, 1, true)
		outputChatBox("** #f0f0f0Canın uyuşturucu almak istiyor. Gözlerin kapanıyor ve dengeni kaybediyorsun. Çok acil uyuşturucuya ihtiyacın var.", 255, 0, 0, true)
		setCameraDrunkLevel(250)
		setTimer(setCameraDrunkLevel, 15000, 1, 0)
	end
end
setTimer(uyusturucuKrizi, 1200000, 0)