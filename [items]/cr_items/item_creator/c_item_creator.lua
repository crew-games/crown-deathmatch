-- ITEM CREATOR BY Farid
wItemList, bItemListClose, isTargetPlayerValid, targetPlayer , targetPlayerID, found , selectedItemName = nil
itemButton = {}
local localPlayer = localPlayer
function showItemList()
	if (exports.cr_integration:isPlayerSeniorAdmin(localPlayer) or exports.cr_integration:isPlayerTrial(localPlayer) or exports.cr_integration:isPlayerScripter(localPlayer)) then
		if not (wItemsList) then
			local width, height = 800,600
			local scrWidth, scrHeight = guiGetScreenSize()
			local x = scrWidth/2 - (width/2)
			local y = scrHeight/2 - (height/2)
			wItemsList = guiCreateWindow(x, y, width, height,"Admin Items Creator v1.0 by Farid",false)
					...
					if (row==-1) or (col==-1) then
						...
						if isTargetPlayerValid then
							...
							if itemId then
								...
								if itemValue ~= "" then
									triggerServerEvent ("itemCreator:spawnItem", localPlayer, localPlayer, targetPlayerID, itemId, itemValue)
								else
