...
local function findVehicleName(value)
	...
	for _, theVehicle in pairs(getElementsByType("vehicle")) do
		if getElementData(theVehicle, "dbid") == value then
			_vehiclecache[value] = exports.cr_global:getVehicleName(theVehicle)
			return _vehiclecache[value]
		end
	end
	return "?"
...
function getItemName(id, value)
	...
	elseif (id == 4 or id == 5) and value then
		local pickup = exports['cr_interior']:findParent(nil, value)
	...
	elseif (id == 165) then --video disc
		...
		if disc > 1 then
			local discData = exports.cr_fakevideo:getFakevideoData(disc)
			if discData then
	...
	if i then
		...
		elseif id == 150 then--ATM card
			...
			if tonumber(itemExploded[2]) > 0 then
				return "Card Number: '" .. itemExploded[1] .. "', Owner: '" .. tostring(exports.cr_cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ") .. "'"
			else
				return "Card Number: '" .. itemExploded[1] .. "', Owner: '" .. tostring(exports.cr_cache:getBusinessNameFromID(math.abs(itemExploded[2]))) .. "'"
			end
		elseif id == 152 then--ID card
			local itemExploded = explode(";", value)
			return "Üzerinde kimlik bilgilerinizin yazdığı kimlik kartı. (( " .. tostring(itemExploded[1]):gsub("_", " ") .. " ))"
		elseif id == 178 then
			local yup = split(value, ":")
...
function getPlayerMaxCarryWeight(element)
	local weightCount = 35 -- Default storage capability in kg 
