...
exports.cr_pool:allocateElement(gioColSphere)
...
function createGio()
	...
	exports.cr_pool:allocateElement(giovanna)
...
--[[local gioChair = createObject(1369, 2442.44140625, -2533.6845703125, 5095.03, 0, 0, 312.97503662109)
	...
	setElementData(giovanna, "rotation", getPedRotation(giovanna), false)
	setElementData(giovanna, "name", "Giovanna Remini")
	setElementData(giovanna, "activeConvo",  0)
	setElementData(giovanna,"talk",true)
	doneDeals = 0
	setElementData(root, "gio.done", 0, false)
		...
		if (gioStat == 1) then
			if (logged==1) then
				if (isElementWithinColShape(thePlayer, gioColSphere)) then
					...
					triggerClientEvent(thePlayer, "startWGio", root, factionType, gioStat)
				end
			end
		end
	end
...
function showGioConvo(stage)
	if (stage) then
		...
		if (allowance) and (allowance == "no")  then
			exports.cr_global:sendLocalText(source, " * Nothing Happens *      ((Unknown_Player))", 255, 51, 102)
			triggerClientEvent(source, "closeGiosWindow", root)
		else
			if (stage == 1) then
				local factionLeader = tonumber(getElementData(source, "factionleader"))
				if (factionLeader == 0) then
					exports.cr_global:sendLocalText(source,"Little Joe shouts: Come back when you got some balls!", 255, 255, 255, 5)
					triggerClientEvent(source, "closeGiosWindow", root)
				else
					exports.cr_global:sendLocalText(source, " *" .. name .. " begins to bang on the door.", 255, 51, 102)				
					exports.cr_global:sendLocalText(source, "Little Joe shouts: Yeah Yeah. I'm coming. Who's there?!", 255, 255, 255, 10)
					
					setElementData (giovanna, "activeConvo",  1)
					talkingToGio = source
...
--					addEventHandler("onPlayerWasted", talkingToGio, resetGioConvoStateDelayed)
					...
					triggerClientEvent(source, "startGioP2", root)
				end
			elseif (stage == 2) then
				exports.cr_global:sendLocalText(source, name .. " shouts: I came here for some guns. Got any supplies?", 255, 255, 255, 5)
				exports.cr_global:sendLocalText(source, "Little Joe shouts: Woooah Brother! You got the wrong guy!", 255, 255, 255, 5)
			...
			elseif (stage == 3) then
				exports.cr_global:sendLocalText(source, name .. " says: It's me. I came here for some business.", 255, 255, 255, 5)
				exports.cr_global:sendLocalText(source, "Giovanna Remini says: I'm all ears. The name is Giovanna, what are you looking for bud?", 255, 255, 255, 5)
			elseif (stage == 4) then
				exports.cr_global:sendLocalText(source, name .. " says: I have all I need.", 255, 255, 255, 5)
				exports.cr_global:sendLocalText(source, "Giovanna Remini says: May the lord be with you my friend.", 255, 255, 255, 5)

				setTimer(resetGioConvoStateDelayed, 600000, 1)
			elseif (stage == 5) then
				exports.cr_global:sendLocalText(source, "Giovanna Remini says: You heard that? It's getting hot around here, I gotta go man. Peace!", 255, 255, 255, 5)
				allowance = "no"
			end
		end
	end
...
addEventHandler("showGioConvo", root, showGioConvo)
...
function giveGioItems(weapon)
	...
	if (count[pfaction] >= 6) then
		...
		if not exports.cr_global:takeMoney(source, cost) then
			exports.cr_global:sendLocalText(source, "Giovanna Remini says: Sorry brother, I'm just not in a charity mood. Come back with the cash.", 255, 255, 255, 5)
			triggerClientEvent(source, "closeGiosWindow", root)

			setTimer(resetGioConvoStateDelayed, 600000, 1)
		else
			count[pfaction] = count[pfaction] + 1
			if not exports.cr_global:giveWeapon(source, itemID, ammo, true) then 
...
addEventHandler("gioGiveItem", root, giveGioItems)
...
function resetGioConvoStateDelayed()
	if talkingToGio then
		...
		triggerClientEvent(talkingToGio, "closeGioWindow", root)
		talkingToGio = nil
		count = {}
		allowance = nil
	end
