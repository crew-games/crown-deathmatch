...
function popupvmPedMenu()
	...
	if not vmOptionMenu then
		...
		triggerServerEvent("vm:ped:start", localPlayer, getElementData(vanna, "name"))
		showCursor(true)
	end
end
addEvent("vm:popupPedMenu", true)
addEventHandler("vm:popupPedMenu", root, popupvmPedMenu)
...
function helpButtonFunction()
	closevmPedMenu()
	triggerServerEvent("vm:ped:help", localPlayer, getElementData(vanna, "name"))
end

function appointmentButtonFunction()
	closevmPedMenu()
	triggerServerEvent("vm:ped:appointment", localPlayer, getElementData(vanna, "name"))
end
