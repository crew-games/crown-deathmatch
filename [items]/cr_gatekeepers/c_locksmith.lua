--[[function displayLocksmithJob()
	outputChatBox("#FF9933Use /copykey [key type][key ID] to duplicate keys.", 255, 194, 15, true)
	outputChatBox("#FF9933The key you want to duplicate must be in your inventory.", 255, 194, 15, true)
end]]

-- by anumaz, for crowndeathmatch, on 2014-11-09
-- configs
...
function createGUI()
	...
	GUIEditor.window[1] = guiCreateWindow(656, 279, 272, 180, "Key Duplicator  - Powered by Tree Technology", false)
...
function duplicateKey(type, id)
	...
	if not exports.cr_global:hasItem(localPlayer, tonumber(keytype), tonumber(id)) then -- checks if you actually have the key on you
	...
	if not exports.cr_global:hasMoney(localPlayer, cost) then -- checks if the player has enough money to get it duplicated
	...
	setTimer(function ()
		if isElement(GUIEditor.window[1]) then
			...
			triggerServerEvent("locksmithNPC:givekey", resourceRoot, localPlayer, keytype, id, cost)
		end
	end, 10000, 1)
end
