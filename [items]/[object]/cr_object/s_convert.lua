			...
			if (result2) then
				...
				if (result3) then
					...
					if (result4) and (result5) then
						exports['cr_interior']:realReloadInterior(tonumber(dimension))
...
--[[Command to delete the interior]]--Farid
function deleteInterior(dimensionID)
	loadDimension(tonumber(dimensionID))
	exports['cr_interior']:realReloadInterior(dimensionID)
	...
	if result1 and result2 then
		exports.cr_global:sendMessageToAdmins("[UCP] Custom interior processing for interior #" .. tostring(dimensionID) .. " has completed!")
	else
		exports.cr_global:sendMessageToAdmins("[UCP] Warning! Could not clean temp objects.")
		return false
	end
end
--addCommandHandler("deltestinterior", deleteInterior, false, false)

--[[Command to save the interior]]--Farid
function saveInterior(dimensionID)
	local result = moveTempToRealDimension(tonumber(dimensionID))
	if result then
		exports.cr_global:sendMessageToAdmins("[UCP] Custom interior has been saved! Now reloading .. ")
		deleteInterior(dimensionID)
	else
		exports.cr_global:sendMessageToAdmins("[UCP] Failed! Something went wrong. Error code: x0983CF")
	end
end
--addCommandHandler("savetestinterior", saveInterior, false, false)

--[[Test the uploaded interior]]--Farid
function testInterior(thePlayer, dimensionID)
	...
	if (count > 0) then
		exports.cr_global:sendMessageToAdmins("[UCP] Loaded " .. count .. " interior custom objects to interior ID#" ..  dimensionID)
		...
		if (result) then
			...
			exports.cr_global:sendMessageToAdmins("[UCP] Failed! Internal error code: x9889DF")
			return false
		end
	else 
		exports.cr_global:sendMessageToAdmins("[UCP] ERROR: No temporary objects found for interior ID#" ..  dimensionID)
		return false
	end
	return true
end
...
function processCustomInterior(thePlayer, commandName, dimensionID, playerUsername)
	if thePlayer and isElement(thePlayer) then
		if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) or exports.cr_integration:isPlayerScripter(thePlayer) then
			if not dimensionID or not tonumber(dimensionID) then
				outputChatBox("KULLANIM: /" .. commandName .. " [interior ID]", thePlayer)
			end
		end
	else 
		if not dimensionID or not tonumber(dimensionID) then
	...
	if thePlayer and isElement(thePlayer) then
		...
		exports.cr_global:sendMessageToAdmins("[UCP] Player " .. playerUsername .. " has uploaded a custom interior for property ID#" .. dimensionID .. ".")
	end
...
--addEventHandler ("onResourceStart", resourceRoot, initiateChecker)