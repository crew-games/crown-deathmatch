...
addEventHandler("item-texture:vehtex", root, vehTex_showGui)
	...
	if button == "left" and state == "up" then
		...
		elseif source == gui.remove then
			...
			if texname ~= "" then
				...
				triggerServerEvent("vehtex:removeTexture", localPlayer, vehicle, texname)
			end
		end
	end
end
...
function vehTex_apply(texname, texurl)
	...
	triggerServerEvent("vehtex:addTexture", localPlayer, vehicle, texname, texurl)
	vehTex_addGui_hide()
end
local extensions = { 
	[".jpg"] = true,
