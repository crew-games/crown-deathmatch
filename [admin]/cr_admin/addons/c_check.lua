...
function OpenCheck(ip, adminreports, donPoints, note, history, warns, transfers, bankmoney, money, adminlevel, hoursPlayed, accountname, hoursAcc)
	...
	if not exports.cr_integration:isPlayerTrialAdmin(localPlayer) then
	...
	if (player == localPlayer) and not exports.cr_integration:isPlayerAdmin1(localPlayer) then
		guiSetText (memo, "-You can not view your own note-")
		guiMemoSetReadOnly(memo, true)
		guiSetEnabled(Button[4], false)
	elseif not exports.cr_integration:isPlayerTrialAdmin(localPlayer) then
...
addEventHandler("onClientRender", root,
	function()
		if guiGetVisible(Window) and isElement(player) then
			local x, y, z = 0, 0, 0
			if not exports.cr_integration:isPlayerSupporter(localPlayer) and getElementAlpha(player) ~= 0 or exports.cr_integration:isPlayerSeniorAdmin(localPlayer) then 
			...
			if vehicle and not exports.cr_integration:isPlayerSupporter(localPlayer) then
			...
			if not exports.cr_integration:isPlayerSupporter(localPlayer) then
...
addEventHandler("cshowAdminHistory", root,
	function(info, targetID)
		if wHist then
			...
			addEventHandler("onClientGUIClick", bremove,
				function(button, state)
					if exports.cr_integration:isPlayerTrialAdmin(localPlayer) or exports.cr_integration:isPlayerSupporter(localPlayer) then
						...
						if row ~= -1 and col ~= -1 then
							...
							if not exports.cr_integration:isPlayerSeniorAdmin(localPlayer) and tonumber(record[8]) ~= getElementData(localPlayer, "account:id") then
