...
function showAdminHistory(target)
	...
	if not (exports.cr_integration:isPlayerTrialAdmin(client) or exports.cr_integration:isPlayerSupporter(client)) then
...
function showOfflineAdminHistory(gameaccountid, name)
	if (exports.cr_integration:isPlayerTrialAdmin(source) or exports.cr_integration:isPlayerSupporter(source)) and tonumber(gameaccountid) then
		...
		if result then
			...
			outputDebugString("cr_admin\showOfflineAdminHistory: Error.")
			outputChatBox("Failed to retrieve history.", source, 255, 0, 0)
		end
	end
end
addEvent("showOfflineAdminHistory", true)
addEventHandler("showOfflineAdminHistory", root, showOfflineAdminHistory)
...
addCommandHandler("history", 
	function(thePlayer, commandName, ...)
		if not (exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer)) then
			if (...) then
