...
addEventHandler("onMakeGun", root, givePlayerGun)
...
addEventHandler("onMakeAmmo", root, givePlayerGunAmmo)
...
function givePlayerItem(thePlayer, commandName, targetPlayer, itemID, ...)
	if exports.cr_integration:isPlayerGeneralAdmin(thePlayer) then
		if not (itemID) or not (...) or not (targetPlayer) then
			...
			if (itemID == 74 or itemID == 150 or itemID == 75 or itemID == 78 or itemID == 2) and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then -- Banned Items
			...
			if targetPlayer then
				...
				if (itemID == 84) and not exports.cr_integration:isPlayerAdmin1(thePlayer) then
				elseif itemID == 114 and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then
		...
		if not (itemID) or not (...) or not (ped) then
			...
			if ped then
				...
				if (itemID == 74 or itemID == 150 or itemID == 75 or itemID == 78 or itemID == 2) and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then -- Banned Items
					----exports.cr_hud:sendBottomNotification(thePlayer, "Banned Items", "Only Lead+ Admin can spawn this kind of item.")
					return false
				elseif (itemID == 84) and not exports.cr_global:isPlayerAdmin1(thePlayer) then
				elseif itemID == 114 and not exports.cr_global:isPlayerSeniorAdmin(thePlayer) then
	...
	if exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then
		if not (...) or not (targetPlayer) then
			...
			if targetPlayer then
				...
				if hoursPlayed > 5 and not exports.cr_integration:isPlayerAdmin1(thePlayer) then
				...
				if newName == targetPlayerName then
					...
					if (name) then
						...
						dbExec(mysql:getConnection(), "UPDATE characters SET charactername = ? WHERE id = ?", newName, dbid)
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
...
addEventHandler("remoteFreezePlayer", root, freezePlayer)
...
addEvent("cr_admin:adminduty", true)
addEventHandler("cr_admin:adminduty", root, adminDuty)

function gmDuty(thePlayer)
	if exports.cr_integration:isPlayerSupporter(thePlayer) then
...
addEvent("cr_admin:gmduty", true)
addEventHandler("cr_admin:gmduty", root, gmDuty)

function vehicleLimit(admin, command, player, limit)
	if exports.cr_integration:isPlayerLeaderAdmin(admin) then
...
function earthquake(thePlayer, commandName)
	if exports.cr_integration:isPlayerDeveloper(thePlayer) then
		...
		for index, arrayPlayer in ipairs(players) do
			triggerClientEvent(root, "doEarthquake", arrayPlayer)
		end
	else
		outputChatBox("[!]#FFFFFF Bu komutu kullanabilmek için gerekli yetkiye sahip değilsiniz.", thePlayer, 255, 0, 0, true)
		playSoundFrontEnd(thePlayer, 4)
...
function unRecovery(thePlayer, commandName, targetPlayer)
	...
	if exports.cr_integration:isPlayerAdmin1(thePlayer) or (factionType==4) then
		if not (targetPlayer) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Karakter Adı / ID]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.cr_global:findPlayerByPartialNick(thePlayer, targetPlayer)
	...
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer) then
		if not targetPlayer then
			...
			if targetPlayer then
				...
				if theVehicle then
					setElementPosition(theVehicle, 2034.1220703125 + math.random(3, 6), -1415.0302734375 + math.random(3, 6), 16.9921875)
					setElementInterior(theVehicle, 0)
					setElementDimension(theVehicle, 0)
				else 
					setElementPosition(targetPlayer, 2034.1220703125 + math.random(3, 6), -1415.0302734375 + math.random(3, 6), 16.9921875)
...
function changeUsername(thePlayer, commandName, username, newUsername)
    if exports.cr_integration:isPlayerManager(thePlayer) then
        if username and newUsername then
            ...
            end, mysql:getConnection(), "SELECT * FROM accounts WHERE username = ?", username)
        else
...
function changePassword(thePlayer, commandName, username, password, passwordAgain)
    if exports.cr_integration:isPlayerManager(thePlayer) then
        if username and password and passwordAgain then
            ...
            outputChatBox("KULLANIM: /" .. commandName .. " [Kullanıcı Adı] [Yeni Şifre] [Yeni Şifre Yeniden]", thePlayer, 255, 194, 14)
        end
    else
        outputChatBox("[!]#FFFFFF Bu komutu kullanabilmek için gerekli yetkiye sahip değilsiniz.", thePlayer, 255, 0, 0, true)
        playSoundFrontEnd(thePlayer, 4)
...
function paraDagit(thePlayer, commandName, amount)
    if exports.cr_integration:isPlayerGeneralAdmin(thePlayer) then
		if amount and tonumber(amount) and tonumber(amount) > 0 then
			...
			if not (amount >= 1000000) then
				for _, player in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
					if getElementData(player, "loggedin") == 1 then
						exports.cr_global:giveMoney(player, amount)
						exports.cr_infobox:addBox(player, "success", "Crown Deathmatch'den herkese $" .. exports.cr_global:formatMoney(amount) .. " hediye!")
					end
				end
				...
				outputChatBox("[!]#FFFFFF Güvenlik amaçlı en fazla 1,000,000$ dağıtabilirsiniz.", thePlayer, 255, 0, 0, true)
				playSoundFrontEnd(thePlayer, 4)
			end
		else
