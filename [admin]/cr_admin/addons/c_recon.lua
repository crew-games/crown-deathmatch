...
function reconPlayer(commandName, targetPlayer)
	...
	if getElementData(localPlayer, "loggedin") == 1 and exports.cr_integration:isPlayerTrialAdmin(localPlayer) or exports.cr_integration:isPlayerSupporter(localPlayer) then
		...
		if not (targetPlayer) then
			...
			if getElementData(targetPlayer, "loggedin") ~= 1 then
				return outputChatBox("[!]#FFFFFF Bu oyuncu karakterine giriş yapmadığı için işlem gerçekleşmedi.", 255, 0, 0, true)
			end
