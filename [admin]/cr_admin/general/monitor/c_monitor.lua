...
addEventHandler("onMonitorPopup", root, showMonitorWindow)
...
function showoMonitorAdd()
	...
	addEventHandler ("onClientGUIClick", guiLogInSubmitButton,  function(button, state)
		if (button == "left") then
			triggerServerEvent("monitor:add", localPlayer, guiGetText(guiUserNameEdit),  guiGetText(guiPasswordEdit))
			destroyElement(guiLogIn)
			guiSetInputEnabled (false)
			 guiLogIn = nil
		end
...
addEventHandler("monitor:oadd", root, showoMonitorAdd)
...
function showoMonitorAdd2()
	...
	addEventHandler ("onClientGUIClick", guiLogInSubmitButton,  function(button, state)
		if (button == "left") then
			triggerServerEvent("monitor:checkUsername", localPlayer, guiGetText(guiUserNameEdit))
			destroyElement(guiLogIn)
			guiSetInputEnabled (false)
			guiLogIn = nil
		end
...
addEventHandler("monitor:oadd2", root, showoMonitorAdd2)
...
function showMonitorEdit(username, charname, monitorContent)
	...
	addEventHandler ("onClientGUIClick", bSave1,  function(button, state)
		if (button == "left") then
			triggerServerEvent("monitor:onSaveEdittedMonitor",localPlayer,localPlayer, username, guiGetText(mMonitorContent), tostring(charname):gsub(" ", "_"))
			destroyElement(wMonitorEdit)
			guiSetInputEnabled (false)
			wMonitorEdit = nil
		end
