...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ Farid
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700, 461+yExtend, "Crown Deathmatch - Yetkili Görevlendirme Arayüzü", false)
...
addEvent("staff.openStaffManager", true)
addEventHandler("staff.openStaffManager", root, openStaffManager)
...
function guiStaffChange()
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if (not hasPlayerAccess(localPlayer)) and (getElementData(localPlayer, "admin_level") > 0) and (not exports.cr_integration:isPlayerSupporterManager(localPlayer)) then
