...
function openFeedBackForm(target)
	...
	gui.window[1] = guiCreateWindow(x, y, w, h, "Crown Deathmatch - Geribildirimi Puanlama", false)
	guiWindowSetMovable(gui.window[1], false)
	guiWindowSetSizable(gui.window[1], false)

	gui.label[1] = guiCreateLabel(10, 28, 319, 33, "Geribildirim kalitesini üstyönetime aktarın, " .. exports.cr_global:getPlayerFullIdentity(playerSource or localPlayer, 1, true) .. " adlı yetkili raporunuza cevap verdi", false, gui.window[1])
	guiLabelSetHorizontalAlign(gui.label[1], "left", true)
	gui.edit[1] = guiCreateEdit(14, 196, 313, 29, "Burayı boş geçmeyin", false, gui.window[1])
	guiEditSetMaxLength(gui.edit[1], 500)

	gui.radiobutton[5] = guiCreateRadioButton(14, 67, 313, 15, "Mükemmel", false, gui.window[1])
	gui.radiobutton[4] = guiCreateRadioButton(14, 92, 313, 15, "Çok İyi", false, gui.window[1])
	gui.radiobutton[3] = guiCreateRadioButton(14, 117, 313, 15, "İyi", false, gui.window[1])
	gui.radiobutton[2] = guiCreateRadioButton(14, 142, 313, 15, "Kötü", false, gui.window[1])
	gui.radiobutton[1] = guiCreateRadioButton(14, 167, 313, 15, "Çok Kötü", false, gui.window[1])
	...
	gui.button[1] = guiCreateButton(14, 240, 150, 49, "Kapat", false, gui.window[1])
	gui.button[2] = guiCreateButton(174, 240, 148, 49, "Gönder", false, gui.window[1])
	...
	exports["cr_items"]:playSoundInvOpen()
end
...
function closeFeedbackForm()
	if gui.window[1] and isElement(gui.window[1]) then
		...
		exports["cr_items"]:playSoundInvClose()
		showCursor(false)
...
function openFeedBackDetails(data)
	if data then
		...
		if #feedbacks > 0 then
			...
			exports.cr_global:centerWindow(gui.window[2])
			...
			local function formatFrom(from)
				if not from then
					...
					return exports.cr_integration:isPlayerSeniorAdmin(localPlayer) and from or "(Hidden)"
				end
			end
...
function openFeedbackEntry(text)
	if text then
		...
		exports.cr_global:centerWindow(gui.window[3])
