...
local content = {}
local thisResourceElement = resourceRoot
...
addEventHandler("onClientRender",root, function ()
	if showTopRightReportBox(localPlayer) and not getElementData(localPlayer, "integration:previewPMShowing") then 
		if (getElementData(localPlayer, "loggedin") == 1) and (getPedWeapon(localPlayer) ~= 43 or not getPedControlState("aim_weapon")) and not isPlayerMapVisible() then
			...
			setElementData(localPlayer, "report-system:dxBoxHeight", heightTemp+hoffset-35)
			
			for i=1, #content do
				if content[i] then
					dxDrawText(content[i][1] or "" , sx-width+10+woffset, (16*i)+hoffset, width-5, 15, tocolor (content[i][2] or 255, content[i][3] or 255, content[i][4] or 255, content[i][5] or 255), content[i][6] or 1, content[i][7] or (exports.cr_fonts:getFont("sf-regular", 9) or "default"), "left", "top", false, false, false, true)
				end
			end
		end
	else
...
function startAutoUpdate()
	...
	elseif exports.cr_integration:isPlayerSupporter(localPlayer) then
