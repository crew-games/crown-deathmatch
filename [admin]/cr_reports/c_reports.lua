...
function showReportMainUI()
	...
	if (logged==1) then
		if (wReportMain==nil)  then
			reportedPlayer = nil
			wReportMain = guiCreateWindow(0.2, 0.2, 0.2, 0.25, "Crown Deathmatch - Yardım Arayüzü", true)
			playSound("sound.mp3")
			scale(wReportMain)
...
function submitReport(button, state)
	if (source==bSubmitReport) and (button=="left") and (state=="up") then
		...
		triggerServerEvent("clientSendReport", localPlayer, reportedPlayer or localPlayer, tostring(guiGetText(tReport)), (guiComboBoxGetSelected(cReportType)+1))
...
function canISubmit()
	...
	--[[local adminreport = getElementData(localPlayer, "adminreport")
	local gmreport = getElementData(localPlayer, "gmreport")]]
...
function clickCloseButton(button, state)
	...
	elseif (button=="left") and (state=="up") then
		...
		triggerEvent("viewF1Help", localPlayer)
	end
end
...
addEventHandler("report:onOpenCheck", root, onOpenCheck)
