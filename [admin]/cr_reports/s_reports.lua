...
function getAdminCount()
	...
	for key, value in ipairs(getElementsByType("player")) do
		if (isElement(value)) then
			...
			if exports.cr_integration:isPlayerSupporter(value) then
				gm = gm + 1
...
function updateReportCount()
	...
	for key, value in ipairs(getElementsByType("player")) do
		if exports.cr_integration:isPlayerStaff(value) then
			...
			elseif exports.cr_integration:isPlayerSupporter(value) then
				str = ":: " .. gmduty .. "/" .. gm .. " SUP"
			else
				str = ""
			end
...
function FaridReportsReminder()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (exports.cr_integration:isPlayerSupporter(value) and god == 1) or (exports.cr_integration:isPlayerTrialAdmin(value) and aod == 1) then
...
addEventHandler("accounts:characters:change", root, playerQuit)
...
addEventHandler("accounts:characters:spawn", root, playerConnect)
...
addEventHandler("clientSendReport", root, handleReport)
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					--[[
					if reportedPlayer == thePlayer and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) and not isAuxiliaryReport(reports[id][7]) then
					...
					--local found = false
					--if not found and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) and isElement(reportHandler) then
					...
					if reportingPlayer == thePlayer and not exports.cr_integration:isPlayerScripter(thePlayer) then
						...
						return false
					--[[
					elseif reportedPlayer == thePlayer and not exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then
						outputChatBox("You better let someone else to handler this report because it's against you.",thePlayer, 255,0,0)
						return false
						]] -- Disabled because staff report is not going to be handled in game anyway / Farid / 2015.1.26
					end
	...
	if (exports.cr_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			if targetAdmin then
				...
				if not (reports[id]) then
					outputChatBox("[!]#FFFFFF Geçersiz Rapor ID.", thePlayer, 255, 0, 0, true)
				elseif (reports[id][5] ~= thePlayer) and not (exports.cr_integration:isPlayerAdmin1(thePlayer)) then
	...
	if exports.cr_integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				if (getElementData(thePlayer, "adminreports") % 50) == 0 then
					exports.cr_items:giveItem(thePlayer, 583, 1)
		...
		if showTopRightReportBox(thePlayer) then
			...
			for i = 1, #reports do
				...
				if report then
					...
					if staff then
						for k,v in ipairs(staff) do
							if (exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerSupporter(thePlayer)) and not seenReport[i] then
...
addEventHandler("onPlayerQuit", root, saveReportCount)
