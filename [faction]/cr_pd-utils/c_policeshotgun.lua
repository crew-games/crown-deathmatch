...
function isPD()
	return getElementData(localPlayer, "faction") == 1 or getElementData(localPlayer, "faction") == 3 -- LSPD faction
end
...
addEventHandler("onClientResourceStart", resourceRoot, bindKeysX)

function enableCooldownX()
	cooldown = 1
	cooldownTimer = setTimer(disableCooldownX, 500, 1)
	toggleControl("fire", false)
	setElementData(localPlayer, "shotgun:reload", true)
end

function disableCooldownX()
	cooldown = 0
	--toggleControl("fire", true)
	setElementData(localPlayer, "shotgun:reload", false)
...
addEventHandler("onClientPlayerWeaponSwitch", root, disableCooldownX)
...
function onTargetPDPedx(element)
	if (isElement(element)) then
		if (getElementType(element)=="ped") and (getElementModel(element)==282) and not (underfire) and (getPedControlState("aim_weapon")) then
			...
			addEventHandler("onClientRender", root, makeCopFireOnPlayerX)
			addEventHandler("onClientPlayerWasted", localPlayer, onDeathX)
		end
	end
end
addEventHandler("onClientPlayerTarget", localPlayer, onTargetPDPedx)
...
function makeCopFireOnPlayerX()
	if (underfire) and (fireelement) then
		...
		if (shotsfired>40) then
			triggerServerEvent("killmebyped", localPlayer, fireelement)
		end
	end
end
...
function onDeathX()
	if (fireelement) and (underfire) then
		...
		fireelement = nil
		underfire = false
		removeEventHandler("onClientRender", root, makeCopFireOnPlayerX)
		removeEventHandler("onClientPlayerWasted", localPlayer, onDeathX)
	end
end