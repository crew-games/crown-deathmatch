...
addEventHandler("onClientVehicleEnter", localPlayer, storeSeat)
...
function fastRope()
	...
	if (vehicle) then
		...
		if (model==497) or (model==563) or (model==487) then
...
addEventHandler("createRope", root, addRope)
...
addEventHandler("onClientRender", root, showRopes)	