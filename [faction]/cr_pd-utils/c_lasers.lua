...
addEventHandler("onClientRender", root, showLaser)
--[[
local tracers = {}
local id = 1
...
function toggleTracers()
	if (getTeamName(getPlayerTeam(localPlayer)) == "Los Santos Police Department") then
...
addEventHandler("onClientPlayerWeaponFire", root, sendTracerRound)
...
addEventHandler("storeTracer", root, storeTracer)
...
addEventHandler("onClientRender", root, displayTracers)]]