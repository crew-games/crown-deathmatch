...
function destroyJailTimer () -- 0001290: PD /release bug
	...
	if (isTimer(theMagicTimer)) then
		...
		setElementData(source, "pd.jailtime", 0, false)
		setElementData(source, "pd.jailtimer", false, false)
		setElementData(source, "pd.jailstation", false, false)
	end
end
addEventHandler ("onPlayerQuit", root, destroyJailTimer)
	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>1441) then
				outputChatBox("KULLANIM: /arrest [Karakter Adı / ID] [Fine] [Jail Time (Minutes 1->1440)] [Crimes Committed]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.cr_global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					elseif targetCol ~= playerCol then
						...
						if (jailTimer) then
							...
							local targetPlayerhasmoney = exports.cr_global:getMoney(targetPlayer, true)
							local amount = tonumber(fine)
							if not exports.cr_global:takeMoney(targetPlayer, amount) then
								finebank = true
								exports.cr_global:takeMoney(targetPlayer, targetPlayerhasmoney)
								local fineleft = amount - targetPlayerhasmoney
								local bankmoney = getElementData(targetPlayer, "bankmoney")
								setElementData(targetPlayer, "bankmoney", bankmoney-fineleft, false)
							end
						
							setElementData(targetPlayer, "pd.jailserved", 0, false)
							setElementData(targetPlayer, "pd.jailtime", jailtime+1, false)
							...
							if restrainedObj then
								...
								if restrainedObj == 45 then -- If handcuffs.. take the key
									...
									exports['cr_items']:deleteAll(47, dbid)
								end
								exports.cr_global:giveItem(thePlayer, restrainedObj, 1)
							...
							setElementData(targetPlayer, "pd.jailstation", targetCol, false)
							
							dbExec(mysql:getConnection(), "UPDATE characters SET pdjail='1', pdjail_time='" .. (jailtime) .. "', pdjail_station='" .. (targetCol) .. "', cuffed = 0, restrainedby = 0, restrainedobj = 0 WHERE id = " .. (getElementData(targetPlayer, "dbid")))
							...
							exports.cr_logs:logMessage("[PD/ARREST] " ..  factionRankTitle .. " " .. username .. " jailed " .. targetPlayerNick .. " for " .. jailtime .. " minute(s). Reason: " .. reason , 30)
						end
					end
				end
			end
		...
		if (timeServed) then
			...
			if (timeLeft<=0) then
				...
				if (getElementData(jailedPlayer, "pd.jailstation") <= 4) then
					-- LSPD
					dbExec(mysql:getConnection(), "UPDATE characters SET pdjail_time='0', pdjail='0', pdjail_station='0' WHERE id=" .. (getElementData(jailedPlayer, "dbid")))
					setElementDimension(jailedPlayer, 2)
				...
				setElementData(jailedPlayer, "pd.jailtimer", false, false)
				setElementData(jailedPlayer, "pd.jailstation", false, false)
			...
			elseif (timeLeft>0) then
				dbExec(mysql:getConnection(), "UPDATE characters SET pdjail_time='" .. (timeLeft) .. "' WHERE id=" .. (getElementData(jailedPlayer, "dbid")))
				local theTimer = setTimer(timerPDUnjailPlayer, 60000, 1, jailedPlayer)
				setElementData(jailedPlayer, "pd.jailtimer", theTimer, false)
			end
		end
	end
end
...
function jailRelease(thePlayer, commandName, targetPlayerNick)
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) or exports.cr_global:isPlayerAdmin1(thePlayer) and getElementData(thePlayer, "adminduty") == 1 then
			if not (targetPlayerNick) then
				outputChatBox("KULLANIM: /release [Karakter Adı / ID]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.cr_global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					if (jailTimer) then
						...
						timerPDUnjailPlayer(targetPlayer)
						exports.cr_logs:logMessage("[PD/RELEASE]" .. username .. " released " .. targetPlayerNick , 30)
					else
