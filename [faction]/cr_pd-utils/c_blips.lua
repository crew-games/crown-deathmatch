...
addEventHandler("onClientResourceStart", resourceRoot,
	function()
		if (getElementData(localPlayer, "faction") == 1 or getElementData(localPlayer, "faction") == 45) and (getElementData(localPlayer) or 0) > 0 then
			for key, value in pairs(getElementsByType("player")) do
				if value ~= localPlayer and not blips[value] and (getElementData(value, "faction") == 1 or getElementData(value, "faction") == 45) and (getElementData(value) or 0) > 0 then
...
addEventHandler("onClientElementDataChange", root,
	function(name)
		if (name == "faction" or name == "duty") and getElementType(source) == "player" then
			...
			elseif (getElementData(localPlayer, "faction") == 1 or getElementData(localPlayer, "faction") == 45) then
