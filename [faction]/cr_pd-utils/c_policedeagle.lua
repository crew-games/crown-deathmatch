...
function isPDDD2()
	return getElementData(localPlayer, "faction") == 1 or getElementData(localPlayer, "faction") == 3
end
...
function enableCooldown()
	...
	setElementData(localPlayer, "deagle:reload", true)
end

function disableCooldown()
	cooldown = 0
	--toggleControl("fire", true)
	setElementData(localPlayer, "deagle:reload", false)
...
addEventHandler("onClientPlayerTarget", localPlayer, onTargetPDPed)
...
function onDeath()
	if (fireelement) and (underfire) then
		...
		removeEventHandler("onClientPlayerWasted", localPlayer, onDeath)
	end
end
