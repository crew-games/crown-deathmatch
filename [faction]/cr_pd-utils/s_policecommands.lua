...
function gorevde(thePlayer, commandName)
    local LSPD = 0
    local LSMD = 0
    local LSSD = 0
    for _, player in ipairs(getElementsByType("player")) do
	    if getElementData(player, "faction") == 1 and (getElementData(player, "loggedin") == 1) then
           LSPD = LSPD + 1		
	    elseif getElementData(player, "faction") == 2 and (getElementData(player, "loggedin") == 1) then
		   LSMD = LSMD + 1
		elseif getElementData(player, "faction") == 3 and (getElementData(player, "loggedin") == 1) then
		   LSSD = LSSD + 1
        end
	end
    outputChatBox("[!]#FFFFFF Görevdeki: LSPD: " .. LSPD .. " | LSMD: " .. LSMD .. " | LSSD: " .. LSSD, thePlayer, 0, 0, 255, true)
end
addCommandHandler("gorevde", gorevde, false, false)
		...
		if factionID == 3 or factionID == 1 then
			if not (targetPlayer) then
				...
				if targetPlayer then
					...
					if (distance <= 5) then
						local badge = factionID == 3 and "LSSD" or "LSPD"
...
addEventHandler("forceElementStreamIn", root, tellNearbyPlayersVehicleStrobesOn)
