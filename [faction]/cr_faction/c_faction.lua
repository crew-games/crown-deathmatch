...
addEventHandler("birlik:panel",root,function(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikseviye,birlikonay)
	if not gFactionWindow2 then
		...
		loadingLabel = guiCreateLabel(0, 0, 1, 1, "Yükleniyor...", true, gFactionWindow)
...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikseviye,birlikonay)
	if (gFactionWindow==nil) then
		...
		guiSetFont(tabs, "default-normal")
		-- Make members list
		gMemberGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabOverview)
		guiSetFont(gMemberGrid, "default-normal")
		...
		local factionPackages = nil --exports.cr_duty:getFactionPackages(factionID)
		if factionPackages and factionType >= 2 then
			colDuty = guiGridListAddColumn(gMemberGrid, "Görev", 0.06)
		end
		birlikSeviyesiLbl = guiCreateLabel(0.8, 0.04, 0.16, 0.05, "Birlik Seviyesi: " .. birlikseviye, true, gFactionWindow)
		...
		if (localPlayerIsLeader) then
			...
			guiSetVisible(tabFinance, false)
		else
			tabFNote = guiCreateTab("Not", tabs)
			fNote = guiCreateMemo(0.01, 0.02, 0.98, 0.87, fnote or "", true, tabFNote)
		...
		guiSetEnabled(gButtonQuit, getElementData(localPlayer, 'faction') == getElementData(factionTheTeam, 'id'))
...
function showrespawn()
	...
	local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Birlik araçlarını yeniden canlandırmak istediğinizden emin misiniz?",true,showrespawnUI)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("[!]#FFFFFF Sıralamalarınız geçersiz karakterler içeriyor, lütfen aşağıdaki gibi karakterler içermediğinden emin olun '@.;", 255, 0, 0, true)
		elseif not (isNumber) then
			outputChatBox("[!]#FFFFFF Ücretleriniz sayı değil, lütfen bir sayı girdiğinizden ve para birimi simgesi olmadığından emin olun.", 255, 0, 0, true)
		else
			local sendRanks = {}
			local sendWages = {}
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			...
			triggerServerEvent("cguiUpdateMOTD", localPlayer, motd)
		end
	end
end
...
function btUpdateNote(button, state)
	if button == "left" and state == "up" then
		triggerServerEvent("faction:note", localPlayer, guiGetText(eNote))
	end
end
...
function btUpdateFNote(button, state)
	if button == "left" and state == "up" then
		triggerServerEvent("faction:fnote", localPlayer, guiGetText(fNote))
	end
end
...
function sendInvite(button, state)
	if (source==bInvite) and (button=="left") and (state=="up") then
		triggerServerEvent("cguiInvitePlayer", localPlayer, invitedPlayer)
	end
end
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		--else
			...
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Gerçekten ayrılmak istiyor musun?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		--[[else]]if (playerName~="") then
			...
			triggerServerEvent("cguiKickPlayer", localPlayer, playerName)
		else
...
function gotPackages(factionPackages)
	...
	wPerkWindow = guiCreateWindow(x, y, width, height, "Birlik avantajları " .. playerName, false)
		...
		function (button, state)
			if (source == bPerkSave) and (button=="left") and (state=="up") then
				if (wPerkWindow) then
					...
					triggerServerEvent("faction:perks:edit", localPlayer, collectedPerks, playerName)
...
function btRespawnOneVehicle(button, state)
	if button == "left" and state == "up" then
		...
		if vehID then
			triggerServerEvent("cguiRespawnOneVehicle", localPlayer, vehID)
		else
	...
	if (button=="left") and (state=="up") and (source==gButtonLeader) then
		...
		elseif (playerName~="") then
			...
			if (currentLevel=="Leader") then
				...
				triggerServerEvent("cguiToggleLeader", localPlayer, playerName, false) -- false = not leader
			else
				guiGridListSetItemText(gMemberGrid, row, tonumber(colLeader), "Leader", false, false)
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
				triggerServerEvent("cguiToggleLeader", localPlayer, playerName, true) -- true = leader
			end
		else
...
function setPhoneNumber(button, state)
	...
	triggerServerEvent("factionmenu:setphone", localPlayer, playerName, num)
...
function savePromotion(button, state)
	if button == "left" and state == "up" and source == promotionButton[1] then
		...
		elseif newRank > currRankNumber then
			...
			triggerServerEvent("cguiPromotePlayer", localPlayer, playerName, newRank, arrFactionRanks[currRankNumber], arrFactionRanks[newRank])
			guiGridListSetSelectedItem(gMemberGrid, row, colRank)
			closePromotion("left", "up")
		elseif newRank < currRankNumber then
			guiGridListSetItemData(gMemberGrid, row, colRank, tostring(newRank))
			triggerServerEvent("cguiDemotePlayer", localPlayer, playerName, newRank, arrFactionRanks[currRankNumber], arrFactionRanks[newRank])
			guiGridListSetSelectedItem(gMemberGrid, row, colRank)
			closePromotion("left", "up")
		else
...
function loadFinance()
	if source == tabFinance then
		if not financeLoaded then	
			...
			triggerServerEvent("factionmenu:getFinance", resourceRoot)
		end
	end
end
...
function fillFinance(factionID, bankThisWeek, bankPrevWeek, bankmoney, vehiclesvalue, propertiesvalue)
	...
	tabWeeklyStatement = guiCreateTab("Haftalık Açıklama", financeTabs)
		...
		guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.cr_global:formatMoney(bankmoney)), false, false)
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Araç", false, false)
		if not vehiclesvalue then vehiclesvalue = 0 end
		guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.cr_global:formatMoney(vehiclesvalue)), false, false)
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Mülker", false, false)
		if not propertiesvalue then propertiesvalue = 0 end
		guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.cr_global:formatMoney(propertiesvalue)), false, false)

		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Toplam", false, false)
		guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.cr_global:formatMoney(bankmoney+vehiclesvalue+propertiesvalue)), false, false)

        local label1 = guiCreateLabel(413, 10, 156, 30, "Birlik finansı bilgileri en fazla 2 hafta öncesine dayanır.", false, tabWeeklyStatement)
	...
	for k,v in ipairs(insertStatement) do
		...
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColLast, "$" .. tostring(exports.cr_global:formatMoney(lastWeek[rowVar])), false, false)
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColThis, "$" .. tostring(exports.cr_global:formatMoney(thisWeek[rowVar])), false, false)
		...
		elseif transTypes then
			...
			for k,v in ipairs(bankThisWeek) do
				if transTypes[v.type] then
					...
					guiGridListSetItemText(transactionDetailsGridlist, row, 5, tostring(exports.cr_global:formatMoney(v.amount)), false, false)
					if v.amount > 0 then
	...
	for k,v in ipairs(bankThisWeek) do
		...
		guiGridListSetItemText(transactionsGridlist, row, 5, tostring(exports.cr_global:formatMoney(v.amount)), false, false)
		if v.amount > 0 then
...
addEventHandler("factionmenu:fillFinance", resourceRoot, fillFinance)
...
-- Made by Chaos for Crown Deathmatch - Custom Duties
Duty = {
    gridlist = {},
    button = {},
    label = {}
	...
	if customEditID ~= 0 then
		...
		for k,v in pairs(customg[customEditID][5]) do
			if tonumber(v[2]) >= 0 then -- Items
				...
				guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["cr_items"]:getItemName(v[2]), false, false) -- Item Name
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["cr_items"]:getItemName(v[2]), false, false) -- Weapon Name
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, tostring(v[3]), false, false) -- Ammo
					guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, { v[1], tonumber(v[2]), v[3], v[4] })
				end
	...
	for k,v in pairs(allowList) do
		if tonumber(v[2]) >= 0 then -- Items
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["cr_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["cr_items"]:getItemDescription(v[2], v[3]), false, false)
			...
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["cr_items"]:getItemName(v[2]), false, false)
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, v[3], false, false)
					guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, { v[1], tonumber(v[2]), v[3] })
				end
			end
...
function addDutyItem()
	...
	if r >= 0 then
		...
		guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["cr_items"]:getItemName(info[2]), false, false) -- Item Name
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			if tonumber(info[1]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["cr_items"]:getItemName(info[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, tostring(info[4]), false, false)
				guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, info)
			end
		end
...
function removeDutyItem()
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["cr_items"]:getItemName(tonumber(info[2])), false, false)
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["cr_items"]:getItemDescription(tonumber(info[2]), info[3]), false, false)
    ...
    addEventHandler("onClientGUIClick", DutyCreate.button[3], function()
		...
		if r >= 0 then
			...
			if tonumber(info[2]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["cr_items"]:getItemName(tonumber(info[2])), false, false) -- Weapon Name
				guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, info)
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, ammo, false, false) -- Ammo
			end
...
function saveGUI()
	if source == DutyCreate.button[2] then -- Main
		...
		if name ~= "" then
			if customEditID ~= 0 then
				triggerServerEvent("Duty:AddDuty", resourceRoot, dutyItems, getElementData(localPlayer, "savedLocations") or customg[customEditID][4], getElementData(localPlayer, "savedSkins") or customg[customEditID][3], name, factionIDg, customEditID)
			else
				triggerServerEvent("Duty:AddDuty", resourceRoot, dutyItems, getElementData(localPlayer, "savedLocations") or {}, getElementData(localPlayer, "savedSkins") or {}, name, factionIDg, customEditID)
			end
			tempLocations = nil
			dutyNewSkins = nil
			dutyItems = nil 
