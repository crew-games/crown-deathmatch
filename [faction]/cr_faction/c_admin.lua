...
function showFactionList(factions)
	if not (wFactionList) then
		wFactionList = guiCreateWindow(0.25, 0.25, 0.5, 0.5, "Birlik Listesi", true)
		...
		local colName = guiGridListAddColumn(gridFactions, "İsim", 0.6)
		local colPlayers = guiGridListAddColumn(gridFactions, "Oyuncular", 0.14)
		local colType = guiGridListAddColumn(gridFactions, "Tip", 0.14)
		...
		addEventHandler("onClientGUIDoubleClick", gridFactions,
			function(button)
				...
				if row ~= -1 and col ~= -1 then
					...
					if button == "left" then
						triggerServerEvent("faction:admin:showplayers", localPlayer, gridID)
					elseif button == "right" then
						triggerServerEvent("faction:admin:showf3", localPlayer, gridID, exports.cr_integration:isPlayerAdmin1(localPlayer))
					end
				else
...
addEventHandler("showFactionList", root, showFactionList)
...
function createAdminDuty(factionT, dutyT)
    ...
    exports.cr_global:guiComboBoxAdjustHeight(DutyAllow.combobox[1], #factionT)
    ...
    exports.cr_global:guiComboBoxAdjustHeight(DutyAllow.combobox[2], 2)
	...
	if selection == 0 then -- Items
		for k,v in pairs(factionTable[key][3]) do
			if tonumber(v[2]) > 0 then
				...
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 2, exports["cr_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 3, exports["cr_items"]:getItemDescription(v[2], v[3]), false, false)
...
function allowItem()
	...
	if not exports['cr_items']:isItem(itemID) and selection == 0 then
