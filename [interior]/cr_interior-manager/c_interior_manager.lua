			...
			function(button)
				if button == "left" then
					...
					addEventHandler("onClientGUIClick", bGo, function(button)
						if button == "left" and wInteriorSearch then
							triggerServerEvent("interiorManager:Search", localPlayer, localPlayer, guiGetText(eSearch))
							destroyElement(wInteriorSearch)
							wInteriorSearch = nil
							-- showCursor(false)
							-- guiSetInputEnabled(false)
				...
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							...
							if triggerServerEvent("interiorManager:delint", localPlayer, localPlayer, gridID) then
								-- closeIntManager()
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:disableInt", localPlayer, localPlayer, gridID) then
								-- closeIntManager()
								-- triggerServerEvent("interiorManager:openit", localPlayer, localPlayer) 
							end
						else
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:gotoInt", localPlayer, localPlayer, gridID) then
								--closeIntManager()
								--triggerServerEvent("interiorManager:openit", localPlayer, localPlayer) 
							end
						else
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:restoreInt", localPlayer, localPlayer, gridID) then
								-- closeIntManager()
								-- triggerServerEvent("interiorManager:openit", localPlayer, localPlayer) 
							end
						else
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:removeInt", localPlayer, localPlayer, gridID) then
								-- closeIntManager()
								-- triggerServerEvent("interiorManager:openit", localPlayer, localPlayer)
							end
						else
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:forceSellInt", localPlayer, localPlayer, gridID) then
								-- closeIntManager()
								-- triggerServerEvent("interiorManager:openit", localPlayer, localPlayer)
							end
						else
			...
			function(button)
				if button == "left" then
					...
					if activeList then
						...
						if row ~= -1 and col ~= -1 then
							local gridID = guiGridListGetItemText(activeList , row, 1)
							if triggerServerEvent("interiorManager:openAdminNote", localPlayer, localPlayer, gridID) then
...
function createCheckIntWindow(result, adminTitle, history, notes1)
	...
	guiSetInputEnabled(true)
	notes = notes1
	checkIntWindow = guiCreateWindow(sx/2-300,sy/2-233,600,466,"Interior Yönetim - Interior Yetkili Not - " .. adminTitle .. " " .. getElementData(localPlayer, "account:username"),false)
	...
	addEventHandler("onClientGUIClick", bToggleInt1,
		function(button)
			if button == "left" then
				triggerServerEvent("interiorManager:disableInt", localPlayer, localPlayer, result[1][1])
			end
		end,
	false)
	...
	if result[1][15] ~= "0" then --deleted
		...
		addEventHandler("onClientGUIClick", bRemoveInt1,
			function(button)
				if button == "left" then
					if triggerServerEvent("interiorManager:removeInt", localPlayer, localPlayer, result[1][1]) then
						closeCheckIntWindow()
					end 
				end
			end,
		...
		addEventHandler("onClientGUIClick", bGotoInt1,
			function(button)
				if button == "left" then
					triggerServerEvent("interiorManager:gotoInt", localPlayer, localPlayer, result[1][1])
				end
			end,
		false)
		...
		addEventHandler("onClientGUIClick", bDelInt1,
			function(button)
				if button == "left" then
					if triggerServerEvent("interiorManager:delint", localPlayer, localPlayer, result[1][1]) then
						triggerServerEvent("interiorManager:checkint", localPlayer, localPlayer, "checkint", result[1][1])
					end
				end
			end,
	...
	addEventHandler("onClientGUIClick", bSave,
		function(button)
			if button == "left" then
				--if triggerServerEvent("interiorManager:saveAdminNote", localPlayer, localPlayer, guiGetText(mAdminNote),result[1][1]) then
