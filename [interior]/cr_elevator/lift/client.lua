...
addEventHandler("lift:hit", root, lift_pickupUse)
...
addEventHandler("onClientPlayerVehicleEnter", root, stopVehicleEntry)
...
addEventHandler("lift:leave", root, lift_pickupLeave)
...
function gotoFloor(button, state)
	if (button=="left") then
		...
		if (row==-1) or (col==-1) then
			...
			local data = exports.cr_global:split(data, ",")
