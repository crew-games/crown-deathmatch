...
function createLift(thePlayer, commandName, lift, floor, ...)
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerScripter(thePlayer)) then
		...
		if (not lift or not floor or not name) then
			outputChatBox("KULLANIM: /" .. commandName .. " [lift ID] [floor] [name]", thePlayer, 255, 194, 14)
...
addEventHandler("onResourceStart", resourceRoot, loadAllLifts)
...
function enterLiftFloor(id)
	...
	if (not isInteriorLocked(dimension)) then	
		if isElement(player) then
			triggerClientEvent(player, "setPlayerInsideInterior", element or root, lifttable, element  or root)
		end
	else
...
addEventHandler("lift:use", root, enterLiftFloor)

function deleteLift(thePlayer, commandName, id)
	if (exports.cr_integration:isPlayerTrialAdmin(thePlayer) or tostring(getElementData(thePlayer, "account:username")) == "Exciter") then
...
function liftOutputMe(text)
	if text then
		exports.cr_global:sendLocalMeAction(client, tostring(text))
	end	
end
addEvent("lift:me", true)
addEventHandler("lift:me", root, liftOutputMe)
