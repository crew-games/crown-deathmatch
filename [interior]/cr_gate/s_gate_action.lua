	...
	if reachedit then
		...
		if not (isGateBusy) then
			...
			if (gateType == 1 or gateType == 3 or gateType == 4 or gateType == 5 or gateType == 7) then
				...
				if (canPlayerControlGate(source, client, password)) or exports.cr_integration:isPlayerSeniorAdmin(client) then
					moveGate(source)
				else
					outputChatBox("[!]#FFFFFF Bu kapı kilitlidir lütfen mülk sahibi ile iletişime geçiniz.", client, 255, 0, 0, true)
				end
			--end
		end
	end
		...
		if gateParameters["state"] then -- its opened, close it
			...
			if gateSound then
				...
				for k,v in ipairs(affectedPlayers) do
					triggerClientEvent(root, "playGateSound", resourceRoot, theGate, false, {x, y, z, int, dim}, gateSound)
				end
			end
			...
			if gateSound then
				...
				affectedByGate[theGate] = affectedPlayers
				for k,v in ipairs(affectedPlayers) do
					triggerClientEvent(root, "playGateSound", resourceRoot, theGate, true, {x, y, z, int, dim}, gateSound)
				end
			end
		end
...
function canPlayerControlGate(theGate, thePlayer, password)
	...
	elseif gateProtection == 3 then
		...
		for _, itemID in ipairs(tempAccess) do
			if (exports.cr_global:hasItem(thePlayer, tonumber(itemID))) then
				return true
			end
		end
	...
	elseif gateProtection == 4 then
		local tempAccess = split(gateParameters["gateSecurityParameters"], " ")
		local hasItem, slotID, itemValue, databaseID = exports.cr_global:hasItem(thePlayer, tonumber(tempAccess[1]))
		if (hasItem) then
	...
	elseif gateProtection == 7 then --for faction ID
		...
		for _, factionID in ipairs(tempAccess) do
			if exports.cr_faction:isPlayerInFaction(thePlayer, tonumber(factionID)) then
				return true
			end 
		end
			...
			if (#orString > 1) then
				...
				for k, v in ipairs(orString) do
					...
					if isNumeric(theItem[1]) then --if number, then treat it as a itemID
						local hasItem, key, value2, value3 = exports.cr_global:hasItem(thePlayer, tonumber(theItem[1]))
						if hasItem then
							...
							if theItem[2] then
								...
								--outputDebugString(tostring(exports['cr_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])))
								if (tonumber(exports['cr_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])) > 0) then
									countOr = countOr + 1
								...
								--if exports["cr_items"]:isBadge(theItem[1]) then
									--if (getElementData(thePlayer, badges[theItem[1]][1])) then
									--if exports["cr_items"]:isWearingBadge(thePlayer, theItem[1]) then
									--	countOr = countOr + 1
						...
						if textFunction == "PILOT" then --check for pilot licenses
							local pilotlicenses = exports['cr_mdc']:getPlayerPilotLicenses(thePlayer) or {}
							...
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										if exports.cr_faction:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2])) then
											countOr = countOr + 1
										end
									else
										if exports.cr_faction:isPlayerInFaction(thePlayer, tonumber(checkFaction[1])) then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.cr_faction:getFactionIDFromName(checkFactionName)
									if factionID then
										if exports.cr_faction:isPlayerInFaction(thePlayer, factionID) then
											countOr = countOr + 1
										end
									end
								end
						...
						elseif textFunction == "FL" or textFunction == "FACTIONLEADER" then --check for faction leadership
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										local isMember, rank, isLeader = exports.cr_faction:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									else
										local isMember, rank, isLeader = exports.cr_faction:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.cr_faction:getFactionIDFromName(checkFactionName)
									if factionID then
										local isMember, rank, isLeader = exports.cr_faction:isPlayerInFaction(thePlayer, factionID)
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
	...
	elseif gateProtection == 9 then --If player has access to the given vehicle
		...
		for _, vehID in ipairs(tempAccess) do
			...
			if veh then
				if (exports.cr_global:isAdminOnDuty(thePlayer) or exports.cr_global:hasItem(thePlayer, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and exports.cr_faction:isPlayerInFaction(thePlayer, getElementData(veh, "faction")))) then
					return true
				end
			end
		end		
	...
	elseif gateProtection == 10 then --keycard
		...
		if (exports.cr_global:hasItem(thePlayer, keycardItemID, gateParameters["gateSecurityParameters"])) then
			return true
		end
	else
