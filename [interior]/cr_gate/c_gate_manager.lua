...
local gatesList1 = {}
local gateID = nil
local version = "v" .. exports.cr_global:getScriptVersion()
...
function createGateManagerWindow(gatesList)
	...
	gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800,600, "Crown Deathmatch - Gate Yöneticis", false)
...
function singleClickedGate(button, state)
	if button == "left" then
		...
		elseif source == bYes then
			triggerServerEvent("delGate", localPlayer,localPlayer, "Farid", gateID)
