...
function setElementDataEx(source, field, parameter, streamtoall, streamatall)
	setElementData(source, field, parameter, streamtoall, streamatall)
end
-- End small hack

function switchGroundSnow(toggle)
	--if getResourceState (getResourceFromName("cr_shader_snow_ground")) == "running" then
...
function findProperty(thePlayer, dimension)
	...
	if dbid and tonumber(dbid) and tonumber(dbid) > 0 then
		dbid = tonumber(dbid) 
		local foundInterior = exports.cr_pool:getElement("interior", dbid)
...
function cleanupProperty(id, donotdestroy)
	if id > 0 then
		if dbExec(mysql:getConnection(), "DELETE FROM dancers WHERE dimension = " .. (id)) then
			local res = getResourceRootElement(getResourceFromName("cr_dancer-system"))
			if res then
		...
		if dbExec(mysql:getConnection(), "DELETE FROM shops WHERE dimension = " .. (id)) then
			local res = getResourceRootElement(getResourceFromName("cr_shop"))
			if res then
				for key, value in pairs(getElementsByType("ped", res)) do
					if getElementDimension(value) == id then
		...
		if dbExec(mysql:getConnection(), "DELETE FROM atms WHERE dimension = " .. (id)) then
			local res = getResourceRootElement(getResourceFromName("cr_bank"))
			if res then
		...
		-- if dbExec(mysql:getConnection(), "DELETE FROM `elevators` WHERE `dimensionwithin` = '" .. (id) .. "' OR `dimension` = '" .. (id) .. "'") then
			-- local res = getResourceRootElement(getResourceFromName("cr_elevator"))
			-- if res then
				-- for key, value in pairs(getElementsByType("elevator", res)) do
					-- if getElementDimension(value) == id then
						-- destroyElement(value)
						-- call(getResourceFromName("cr_elevator"), "reloadOneElevator", value) 
					-- end
				-- end
			-- end
		-- end
		
		local resE = getResourceRootElement(getResourceFromName("cr_elevator"))
		if resE then
			call(getResourceFromName("cr_elevator"), "delElevatorsFromInterior", "Farid" , "PROPERTYCLEANUP",  id) 
		end
		
		if not donotdestroy then
			local res1 = getResourceRootElement(getResourceFromName("cr_object"))
			if res1 then
				exports['cr_object']:removeInteriorObjects(tonumber(id))
			end
		end
	...
	if dbid > 0 then
		...
		elseif interiorType ~= 3 and commandName == "unrent" then
			...
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") or (getElementData(thePlayer, "factionleader") > 0 and interiorStatus[INTERIOR_FACTION] == getElementData(thePlayer, "faction")) then
				...
				exports.cr_logs:dbLog(thePlayer, 37, { "in" .. tostring(dbid) } , "SELLPROPERTY " .. dbid)
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney, CLEANUP)
	...
	if query then
		...
		if safeTable[dbid] then
			...
			if safe then
				exports['cr_items']:clearItems(safe)
				destroyElement(safe)
				safeTable[dbid] = nil
			end
		end
		...
		if interiorType == 0 or interiorType == 1 then
			...
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				if givemoney then
					...
					if exports.cr_global:takeMoney(gov, money, true) then
						exports.cr_bank:addBankTransactionLog(-getElementData(gov, "id"), interiorStatus[INTERIOR_OWNER], money, 3 , "Interior sold to Government", getElementData(interiorElement, "name") .. " (ID: " .. getElementData(interiorElement, "dbid") .. ")")
					end
				end
				
				if showmessages then
					if CLEANUP == "FORCESELL" then
						exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. exports.cr_global:getPlayerFullIdentity(thePlayer) .. " has force-sold interior #" .. dbid .. " (" .. getElementData(interiorElement,"name") .. ").") 
					else
						outputChatBox("Mülkünüzü sattınız " .. exports.cr_global:formatMoney(money)  .. "$", thePlayer, 0, 255, 0)
					end
				end
				
				-- take all keys
				call(getResourceFromName("cr_items"), "deleteAll", interiorType == 0 and 4 or 5, dbid)
			...
			elseif interiorStatus[INTERIOR_FACTION] == getElementData(thePlayer, "faction") then
				local money = math.ceil(interiorStatus[INTERIOR_COST] * 2/3)		
				local faction = exports.cr_faction:getTeamFromFactionID(interiorStatus[INTERIOR_FACTION])
				if givemoney and faction then
					exports.cr_global:giveMoney(faction, money)
					if exports.cr_global:takeMoney(gov, money, true) then
						exports.cr_bank:addBankTransactionLog(-getElementData(gov, "id"), -interiorStatus[INTERIOR_FACTION], money, 3 , "Interior sold to Government", getElementData(interiorElement, "name") .. " (ID: " .. getElementData(interiorElement, "dbid") .. ")")
					end
				end
				...
				if showmessages then
					if CLEANUP == "FORCESELL" then
						exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. exports.cr_global:getPlayerFullIdentity(thePlayer) .. " has force-sold interior #" .. dbid .. " (" .. getElementData(interiorElement,"name") .. ").") 
					elseif faction then
						outputChatBox("Mülkünüzü sattınız " .. exports.cr_global:formatMoney(money) .. "$ (Bankaya havale yapıldı: '" .. getTeamName(faction) .. "')", thePlayer, 0, 255, 0)
					end
				end
...
function unownProperty(intid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the publicSellProperty however, it's simplier. / Farid / 2015.1.11
	...
	call(getResourceFromName("cr_items"), "deleteAll", 4 , intid)
	call(getResourceFromName("cr_items"), "deleteAll", 5 , intid)
	...
	exports.cr_logs:dbLog("SYSTEM", 37, { "in" .. intid } , reason and reason or "FORCESELL")
	exports["cr_interior-manager"]:addInteriorLogs(intid, reason and reason or "Forcesold by SYSTEM")
		...
		elseif not targetPlayerName then
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 10 and getElementDimension(targetPlayer) == getElementDimension(thePlayer) then
					if not exports.cr_global:canPlayerBuyInterior(targetPlayer) then
					...
					if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") or exports.cr_integration:isPlayerSeniorAdmin(thePlayer) then
						if getElementData(targetPlayer, "dbid") ~= interiorStatus[INTERIOR_OWNER] then
							if exports.cr_global:hasSpaceForItem(targetPlayer, 4, dbid) then
								...
								if query then							
									...
									call(getResourceFromName("cr_items"), "deleteAll", 4, dbid)
									call(getResourceFromName("cr_items"), "deleteAll", 5, dbid)
									exports.cr_global:giveItem(targetPlayer, keytype, dbid)
									...
									exports.cr_logs:dbLog(thePlayer, 37, { targetPlayer, "in" .. tostring(dbid) } , "SELLPROPERTY " .. getPlayerName(thePlayer) .. " => " .. targetPlayerName)
									local adminID = getElementData(thePlayer, "account:id")
									
									realReloadInterior(dbid, {targetPlayer, thePlayer})
									exports["cr_interior-manager"]:addInteriorLogs(dbid, commandName .. " to " .. targetPlayerName .. "(" .. getElementData(targetPlayer, "account:username") .. ")", thePlayer)
								else
...
function realReloadInterior(interiorID, updatePlayers)
	local dbid, entrance, exit, inttype, interiorElement = exports['cr_interior']:findProperty(false, interiorID)
	if dbid > 0 then
		if safeTable[dbid] then
	...
	dbQuery(
		function(qh)
			...
			if rows > 0 then
				...
				--Keyless Digital Door Lock / Farid
				...
				whatToReturn = true
				exports.cr_pool:allocateElement(interiorElement, tonumber(row.id), true)
				if massLoad then
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	if not exports.cr_global:canPlayerBuyInterior(player) then
	...
	elseif not exports.cr_global:hasSpaceForItem(player, 4, 1) then
		outputChatBox("Envanterinizde anahtar için yeterli alan yok.", player, 255, 0, 0)
		return
	end
	
	if exports.cr_global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Tebrikler! Az önce bu evi satın aldınız $" ..  exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.cr_global:giveMoney(getTeamFromName("Los Santos President"), cost)
		elseif (isRentable) then
			outputChatBox("Tebrikler! Az önce bu evi kiraladınız $" ..  exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Tebrikler! Az önce bu işyerini aldınız $" .. exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.cr_global:giveMoney(getTeamFromName("Los Santos President"), cost)
		end
		...
		call(getResourceFromName("cr_items"), "deleteAll", 4, pickupid)
		call(getResourceFromName("cr_items"), "deleteAll", 5, pickupid)
		
		if (isHouse) or (isRentable) then
			exports.cr_global:giveItem(player, 4, pickupid)
		else
			exports.cr_global:giveItem(player, 5, pickupid)
		end
		exports.cr_logs:dbLog(thePlayer, 37, { "in" .. tostring(pickupid) } , "BUYPROPERTY $" .. cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["cr_interior-manager"]:addInteriorLogs(pickupid, "Alındı ​​/ Kiralandı, $" .. exports.cr_global:formatMoney(cost) .. ", " .. getPlayerName(thePlayer), thePlayer)
	else
		outputChatBox("Üzgünüz, bu mülkü satın alamazsınız.", player, 255, 194, 14)
	end
end

function buypropertyForFaction(interior, cost, isHouse) --Farid
	local can, reason = exports.cr_global:canPlayerFactionBuyInterior(source)
	if not can then
		outputChatBox(reason, source, 255, 0, 0)
		return
	end
	local theFaction = can
	if not exports.cr_global:takeMoney(theFaction, cost) then
	...
	exports.cr_global:giveMoney(gov, cost)
	exports.cr_bank:addBankTransactionLog(-factionId, -(getElementData(gov, "id")), cost, 3 , "Mülk Satın Alma", intName .. " (ID: " .. intId .. ")")
	...
	if not dbExec(mysql:getConnection(), "UPDATE interiors SET owner='-1', faction='" .. factionId .. "', locked=0, lastused=NOW() WHERE id='" .. intId .. "'") then
		exports.cr_global:giveMoney(theFaction, cost)
		exports.cr_global:takeMoney(gov, cost)
		outputChatBox("Internal error code 334INT2.", source, 255, 0, 0)
		return false
	end
	local factionName = getTeamName(theFaction):gsub("_", " ")
	outputChatBox("Tebrikler! Bu mülkü az önce satın aldınız '" .. factionName .. "' $" .. exports.cr_global:formatMoney(cost) .. ".", source, 255, 194, 14)
	local interiorEntrance = getElementData(interior, "entrance")

	call(getResourceFromName("cr_items"), "deleteAll", isHouse and 4 or 5, intId)
	exports.cr_global:giveItem(source, isHouse and 4 or 5, intId)
	
	exports.cr_logs:dbLog(source, 37, { "in" .. tostring(intId) } , "BUYPROPERTY $" .. cost .. " FOR FACTION '" .. factionName .. "'")
	realReloadInterior(tonumber(intId))
	exports["cr_interior-manager"]:addInteriorLogs(intId, "'" .. factionName .. "', $" .. exports.cr_global:formatMoney(cost) .. ", " .. getPlayerName(source), source)
	...
	exports.cr_achievement:playSoundFx(source)
	return true
end
...
function buyInteriorCash(player, pickup, cost, isHouse, isRentable)
	...
	if exports.cr_global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Tebrik ederiz! Bu evi yeni aldınız $" .. exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.cr_global:giveMoney(getTeamFromName("Los Santos President"), cost)
		elseif (isRentable) then
			outputChatBox("Tebrik ederiz! Bu evi yeni aldınız... $" .. exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Tebrik ederiz! Için bu işi aldınız $" .. exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.cr_global:giveMoney(getTeamFromName("Los Santos President"), cost)
		end
		...
		call(getResourceFromName("cr_items"), "deleteAll", isHouse and 4 or 5, pickupid)
		exports.cr_global:giveItem(player, isHouse and 4 or 5, pickupid)
		
		exports.cr_logs:dbLog(player, 37, { "in" .. tostring(pickupid) } , "BUYPROPERTY $" .. cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["cr_interior-manager"]:addInteriorLogs(pickupid, "Bought/rented, $" .. exports.cr_global:formatMoney(cost) .. ", " .. getPlayerName(player), player)
...
function buyInteriorToken(player, pickup, cost, isHouse, isRentable)
	if not exports.cr_global:canPlayerBuyInterior(player) then
		outputChatBox("Azami iç mekan sayısına zaten ulaştınız. Forum üzerinden ek slot satın alabilirsiniz. (" .. exports.cr_installer:getDetails("website") .. ")", player, 255, 0, 0)
		return
	end
	...
	if exports.cr_global:takeItem(player, 262, 1) then
		local charid = getElementData(player, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		...
		elseif (isRentable) then
			outputChatBox("Tebrikler! Az önce bu mülkü kiraladınız $" ..  exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Tebrikler! Az önce bu işyerini satın aldınız $" ..  exports.cr_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		end
		...
		exports.cr_logs:dbLog(player, 37, { "in" .. tostring(pickupid) } , "BUYPROPERTY $" .. cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["cr_interior-manager"]:addInteriorLogs(pickupid, "Alındı ​​/ Kiralandı, $" .. exports.cr_global:formatMoney(cost) .. ", " .. getPlayerName(player), player)
	--	triggerClientEvent(player, "createBlipAtXY", player, interiorEntrance[INTERIOR_TYPE], interiorEntrance[INTERIOR_X], interiorEntrance[INTERIOR_Y])
	else
		outputChatBox("Üzgünüz, bu mülkü satın alamazsınız.", player, 255, 194, 14)
	end
...
function buyInteriorBank(player, pickup, cost, isHouse, isRentable)
	...
	if not exports.cr_bank:takeBankMoney(player, cost) then
		...
		exports.cr_bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 2 , "Mülk Satın Alma", intName .. " (ID: " .. pickupid .. ")")
...
function enterInterior()
	if source and client then
		...
		elseif isInteriorForSale(source) then
			...
			local neighborhood = exports.cr_global:getElementZoneName(source)
...
function setPlayerInsideInterior(theInterior, thePlayer, teleportTo)
	...
	setElementData(theInterior, "lastused", exports.cr_datetime:now(), true)

	--Alright, it's time to give admins some clues of what just happened
	exports.cr_logs:dbLog("SYSTEM", 31, { theInterior, thePlayer } , enter and "ENTERED" or "EXITED")
	exports["cr_interior-manager"]:addInteriorLogs(dbid, enter and "Entered" or "Exited", thePlayer)
...
function addSafeAtPosition(thePlayer, x, y, z, rotz)
	...
	elseif dbid >= 20000 then -- Vehicle Interiors
		...
		if exports['cr_vehicle']:getSafe(vid) then
			outputChatBox("Bu mülkte zaten bir kasa var. Taşımak için /movesafe yazın.", thePlayer, 255, 0, 0)
			return 1
		elseif exports.cr_global:hasItem(thePlayer, 3, vid) then
			z = z - 0.5
			rotz = rotz + 180
			if dbExec(mysql:getConnection(), "UPDATE vehicles SET safepositionX='" .. x .. "', safepositionY='" .. y .. "', safepositionZ='" .. z .. "', safepositionRZ='" .. rotz .. "' WHERE id='" .. vid .. "'") then
				if exports['cr_vehicle']:addSafe(vid, x, y, z, rotz, interior) then
					return 0
				end
			end
			return 1
	...
	elseif dbid >= 19000 then -- temp vehicle interiors
		return 2
	elseif ((exports.cr_global:hasItem(thePlayer, 5, dbid) or exports.cr_global:hasItem(thePlayer, 4, dbid))) then
		if safeTable[dbid] then
			outputChatBox("Bu mülkte zaten bir kasa var. Taşımak için /movesafe yazın.", thePlayer, 255, 0, 0)
			return 1
		else
			...
			call(getResourceFromName("cr_items"), "clearItems", tempobject)
			return 0
		end
	end
	return 3
...
function moveSafe (thePlayer, commandName)
	...
	local dbid = getElementDimension(thePlayer)
	local interior = getElementInterior(thePlayer)
	if (dbid < 19000 and (exports.cr_global:hasItem(thePlayer, 5, dbid) or exports.cr_global:hasItem(thePlayer, 4, dbid))) or (dbid >= 20000 and exports.cr_global:hasItem(thePlayer, 3, dbid - 20000)) then
		z = z - 0.5
		rotz = rotz + 180
		if dbid >= 20000 and exports['cr_vehicle']:getSafe(dbid-20000) then
			local safe = exports['cr_vehicle']:getSafe(dbid-20000)
...
local function hasKey(source, key)
	if exports.cr_global:hasItem(source, 4, key) or exports.cr_global:hasItem(source, 5,key) then
		return true, false
	else
...
function lockUnlockHouseEvent(player, checkdistance)
	...
	local possibleElevators = exports.cr_pool:getPoolElementsByType("elevator")
	...
	if found and itemValue then
		...
		if getElementData(interiorElement, "keypad_lock") then
			if not (exports.cr_integration:isPlayerTrialAdmin(source) and getElementData(source, "duty_admin") == 1) then
				--exports.cr_hud:sendBottomNotification(source, "Keyless Digital Door Lock", "This door is keyless, you must use the keypad to access it.")
				return false
			end
		end
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.cr_global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. adminTitle .. " " ..  getPlayerName(source):gsub("_", " ").. " (" .. adminUsername .. ") has unlocked Interior ID #" .. itemValue .. " without key.")
					--exports.cr_global:sendLocalText(source, " * Kapı şuan kilidi açık *", 223, 174, 255, 30, {}, true)
					--exports["cr_interior-manager"]:addInteriorLogs(itemValue, "unlock without key", source)
				end
			else
				triggerEvent('sendAme', source, "kilidi açmak için anahtarı kapıya koyar.")
			end
			exports.cr_logs:dbLog(source, 31, {  "in" .. tostring(itemValue) }, "UNLOCK INTERIOR")
		else --shit
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.cr_global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. adminTitle .. " " ..  getPlayerName(source):gsub("_", " ").. " (" .. adminUsername .. ") has locked Interior ID #" .. itemValue .. " without key.")
					--exports.cr_global:sendLocalText(source, " * Kapı şuan kilitli *", 223, 174, 255, 30, {}, true)
					--exports["cr_interior-manager"]:addInteriorLogs(itemValue, "lock without key", source)
				end
			else 
				triggerEvent('sendAme', source, "kilitlemek için anahtarı kapıya yerleştirir.")
			end
			exports.cr_logs:dbLog(source, 31, {  "in" .. tostring(itemValue) }, "LOCK INTERIOR")
		end
...
function doorUnlockSound(house, source)
	if (house) then
		if (getElementType(house) == "interior") then
			...
			for index, nearbyPlayer in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
				...
				if locked == 0 then
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.cr_global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. adminTitle .. " " ..  getPlayerName(source):gsub("_", " ").. " (" .. adminUsername .. ") has unlocked Interior ID #" .. id .. " without key.")
							exports.cr_global:sendLocalText(source, " * Kapı kilidi şuan açık *", 223, 174, 255, 30, {}, true)
							exports["cr_interior-manager"]:addInteriorLogs(id, "unlock without key", source)
						end
					else
						triggerEvent('sendAme', source, " kilidi açmak için anahtarı kapıya koyar")
					end
					exports.cr_logs:dbLog(source, 31, {  "in" .. tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.cr_global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.cr_global:sendMessageToAdmins("[INTERIOR]: " .. adminTitle .. " " ..  getPlayerName(source):gsub("_", " ").. " (" .. adminUsername .. ") has locked Interior ID #" .. id .. " without key.")
							exports.cr_global:sendLocalText(source, " * Kapı şuan kilitli *", 223, 174, 255, 30, {}, true)
							exports["cr_interior-manager"]:addInteriorLogs(id, "lock without key", source)
						end
					else
						triggerEvent('sendAme', source, " kilitlemek için anahtarı kapıya yerleştirir.")
					end
					exports.cr_logs:dbLog(source, 31, {  "in" .. tostring(id) }, "LOCK INTERIOR")
				end
...
function client_requestHUDinfo()
	...
	local interiorOwnerName = exports['cr_cache']:getCharacterName(interiorStatus[INTERIOR_OWNER]) or "None"
