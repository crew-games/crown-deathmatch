...
local interiorSettings = {
	--label, id, type
	{"Zamanı Ayarla", "time", "options", options={"auto", "day", "night"} },
	{"Yerel OOC kanalını kapat", "ooc", "bool"},
}
...
function openSettingsGui(element, playerInterior, interiorID, data)
	...
	if (localPlayerInterior == playerInterior and localPlayerDimension == interiorID) then
		if (playerInterior > 0 and interiorID > 0) then --doublecheck valid interior
			...
			window = guiCreateWindow(223, 185, 324, 503, "Mülk Ayarları", false)
			...
			local btn = guiCreateButton(10, h-30, w-20, 20, "Kaydet & Kapat", false, window)
...
addEventHandler("interior:settingsGui", root, openSettingsGui)
